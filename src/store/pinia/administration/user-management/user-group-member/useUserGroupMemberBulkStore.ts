import { BULK_API_URL, EXPORT_TEMPLATE_URL, UPLOAD_API_URL } from "./urls";
...
import {
  BulkItem
} from "@/core/types/entities/administration/user-management/user-group-member/BulkItem";
import {
  ValidatedItem
...
export const useUserGroupMemberBulkStore = defineStore({
  id: "UserGroupMemberBulk",
  state: () => {
    return {
  ...
  actions: {
    ...
    async exportTemplate() {
      ...
      try {
        const response: AxiosResponse<Blob> = await ApiService.getBlob(`${EXPORT_TEMPLATE_URL}`, "", new URLSearchParams(params).toString());
