...
export const useGeneralFormStore = defineStore({
  ...
  state: () => {
    return {
      ...
      stateSelectedFitterShift: {} as ShiftList,
      stateTimeStamp: '',
  ...
  actions: {
          ...
          updateParams: [
            {
              ...
              propertyParams: [
                {
                  propertyName: 'log',
                  propertyValue: JSON.stringify(this.stateFitterInfo)
                },
              ]
            }
          ],
    ...
    setPayloadId(id: string, employeeId: string, name: string) {
        ...
        this.statePayload.localStatus = this.stateGeneralForm.status
    },
    ...
    async updateGeneralForm() {
        ...
        if (isFormComplete) {
          let status = this.stateGeneralForm.status
          if (!isOfflineOrSlowInternetConnection()) {
            status = await this.getServiceSheetHeaderKeyValue('status') as string
          }
          if (status.toString().toLowerCase() == 'open' || status.toString().toLowerCase() == 'on progress') {
            try {
              let response
              if (!isOfflineOrSlowInternetConnection()) {
                response = await ApiService.post(`${GENERAL_SERVICE_SHEET_API_URL}?${new URLSearchParams(params).toString()}`, this.statePayload as AxiosRequestConfig);
                ...
                // dump data to local
                this.stateGeneralForm.status = 'On Progress'
                this.stateGeneralForm.serviceStart = AESTCurrentDateTime()
                response = await this.updateGeneralFormToLocalDB(this.statePayload)
              }
    ...
    async updateGeneralFormToLocalDB(payload) {
      await db.pendingTask.add({
        module: 'serviceForm',
        section: 'E-Form',
        type: 'General',
    ...
    toggleIsSmuCameraDisabled(status) {
      this.stateIsSmuCameraDisabled = status
    },
    ...
    async getServiceSheetHeaderKeyVal(key, name) {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_get_${name}_data`, {
          errorMessage: error
        })
        updateLoggedInStatusFromAPIResponse(error)
        console.log(error);
    ...
    async getServiceSheetHeaderImages(key: string) {
      const body = {
        id: this.stateGeneralForm.id,
        keyValue: 'GENERAL',
        propertyName: key,
    ...
    async UpdateServiceEndMechanic(payload) {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_update_service_end_fitter`, {
          errorMessage: error
        })
        updateLoggedInStatusFromAPIResponse(error)
        sendErrorEvent('IRONS', error);
    ...
    async getSMUTolerance() {
      ...
      try {
        ...
        if (isNoNetwork) {
          await this.getSMUToleranceFromLocalDB()
        } else {
    ...
    async getSMUToleranceFromLocalDB() {
    ...
    async getServerTimestamp() {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_get_time_stamp_data`, {
          errorMessage: error
        })
        updateLoggedInStatusFromAPIResponse(error)
        sendErrorEvent('IRONS', error);
    ...
    async getShift(shift) {
      const selectedShift = this.stateShiftList.find((shiftItem) => {
        return shiftItem.shift == shift
      })
      this.stateSelectedFitterShift = selectedShift!
      return selectedShift
    },
    updateSMUImages(newImages) {
      this.stateGeneralForm.imageEquipment = newImages
    },
    updateSwingImages(newImages) {
      let arr = [] as any[] | string
      if (this.generalForm.swingHourImages && this.generalForm.swingHourImages != '') {
        arr = this.generalForm.swingHourImages as string[]
      }
      arr = [
        ...arr,
        ...newImages
      ]
      this.stateGeneralForm.swingHourImages = arr
    },
    updateTravelImages(newImages) {
      let arr = [] as any[] | string
      if (this.generalForm.travelHourImages && this.generalForm.travelHourImages != '') {
        arr = this.generalForm.travelHourImages as string[]
      }
      arr = [
        ...arr,
        ...newImages
      ]
      this.stateGeneralForm.travelHourImages = arr
    },
    deleteSMUPic(deletedImage) {
      let arr = this.generalForm.imageEquipment as ImageInfo[]
      arr = arr.filter((image) => {
        return image.filename != deletedImage.filename
      })
      this.generalForm.imageEquipment = arr
    },
    deleteSwingPic(deletedImage) {
      let arr = this.generalForm.swingHourImages as ImageInfo[]
      arr = arr.filter((image) => {
        return image.filename != deletedImage.filename
      })
      this.generalForm.swingHourImages = arr
    },
    deleteTravelPic(deletedImage) {
      let arr = this.generalForm.travelHourImages as ImageInfo[]
      arr = arr.filter((image) => {
        return image.filename != deletedImage.filename
      })
      this.generalForm.travelHourImages = arr
    },
    ...
    async updateServiceSheetHeaderToTrue() {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_update_service_sheet_is_deleted`, {
          errorMessage: error
        })
        sendErrorEvent('IRONS', error);
    ...
    async getShiftListData() {
      ...
      try {
        ...
        // dump timezone
        await db.timeZone.clear()
        await db.timeZone.add({
          timeZone: clone(timeZoneResponse.data.result.content.value)
        })
        ...
        // dump timezone desc
        await db.timeZoneDesc.clear()
        await db.timeZoneDesc.add({
          timeZoneDesc: clone(timeZoneDescResponse.data.result.content.value)
        })
        ...
        // dump shift
        this.stateShiftList.forEach(async (shift) => {
          const existing = await db.shift.where('shift').equals(shift.shift).first()
          if (!existing) {
            await db.shift.add(clone(shift))
          }
        })
      } catch (error) {
        sendCustomEvent(`fe_event_error_get_fitter_time_zone_data`, {
          errorMessage: error
        })
        updateLoggedInStatusFromAPIResponse(error)
        sendErrorEvent('IRONS', error);
    ...
    async getShiftFromLocalDB() {
      const timeZone = await db.timeZone.limit(1).first()
      this.stateTimeZone = timeZone!.timeZone
      const timeZoneDesc = await db.timeZoneDesc.limit(1).first()
      this.stateTimeZoneDesc = timeZoneDesc!.timeZoneDesc
    ...
    async postCheckBeforeTruckItem(updateParam: UpdateParams, itemKey: string, isReset?: boolean) {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_post_check_before_truck`, {
          errorMessage: error
        })
        updateLoggedInStatusFromAPIResponse(error)
        sendErrorEvent('IRONS', error);
    ...
    async updateCheckBeforeTruckToLocalDB(updateParam: UpdateParams, itemKey: string, reason = "", isReset?: boolean) {
      const eformStore = useEFormStore()
      const updatedBy = eformStore.employee
      let updateByValue = JSON.stringify(updatedBy)
      if (isReset) {
      ...
      checkedItem!.updatedDate = AESTCurrentDateTime()
      if (!isUndefined(checkedItem?.reason)) {
        checkedItem!.reason = reason
      }
      checkedItem!.updatedBy = this.eformStore.employee
      await db.serviceSheetDetail.put(cloneDeep({
        id: this.generalForm.workOrder,
        general: this.stateGeneralForm,
        serviceSheet: this.eformStore.stateServiceSheets
    ...
    async updateSwingTravelValueToBE(propertyName, propertyValue) {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_post_swing_travel_data`, {
          errorMessage: error
        })
        sendErrorEvent('IRONS', error);
    ...
    async updateSMUValueToBE(paramData: passingParameterUpdateSMU) {
      ...
      try {
        ...
        sendCustomEvent(`fe_event_error_post_smu_value`, {
          errorMessage: error
        })
        sendErrorEvent('IRONS', error);
        throw error
    ...
    async updateSMUImageToBE(data = {
      ...
      try {
        return await ApiService.post(`${GENERAL_SERVICE_SHEET_API_URL}?${new URLSearchParams(params).toString()}`, payload as AxiosRequestConfig);
      } catch (error) {
    ...
    async updateTravelSwingImageToLocal(type: string, images: ImageInfo[]) {
      const payload = {
        employee: this.eformStore.stateEmployee,
        id: this.generalForm.id,
        updateParams: [
          {
            keyValue: 'GENERAL',
    ...
    async getHmOffsetValue(site) {
      ...
      if (eformStore.generalForm.smu) {
        ...
        try {
          ...
          sendCustomEvent(`fe_event_error_get_hm_offset`, {
            errorMessage: e
          })
    ...
    async getHmOffsetValueFromLocalDB() {
      this.stateHmOffset = 0
      const eformStore = useEFormStore()
      if (eformStore.generalForm.smu) {
        if (isUndefined(eformStore.generalForm.hmOffset) || isNull(eformStore.generalForm.hmOffset)) {
        ...
        // get API
        try {
          const hmOffset = await db.hmOffset.where({
            equipmentNo: eformStore.unitNumber
          }).toArray()
          if (hmOffset.length == 0) this.stateHmOffset = 0
          else if (hmOffset.length > 0) {
            this.stateHmOffset = hmOffset[0].hmOffset
          }
        } catch (e) {
          console.log("error get hm offset", e);
        }
    ...
    async updateSMUValueToLocal(paramData: passingParameterUpdateSMU) {
      const { smu, hmOffset, smuDate, smuBy } = paramData
      const payload = {
        employee: this.eformStore.stateEmployee,
        id: this.generalForm.id,
      ...
      if (hmOffset != "") {
        this.stateGeneralForm.hmOffset = hmOffset
        payload.updateParams[0].propertyParams.push({
          propertyName: 'hmOffset',
          propertyValue: hmOffset
        })
      ...
      const existingTask = db.pendingTask.where({
        ...
        syncStatus: 'Pending',
        itemKey: `${this.eformStore.stateGeneralForm.workOrder}-SMU`
      });
      if (existingTask) {
    ...
    async updateSMUImageToLocal(val, hmOffset = "") {
      const payload = {
        employee: this.eformStore.stateEmployee,
        id: this.generalForm.id,
        updateParams: [
    ...
    async updateSwingValueToLocal(val, propertyName) {
      const payload = {
        employee: this.eformStore.stateEmployee,
        id: this.generalForm.id,
        updateParams: [
