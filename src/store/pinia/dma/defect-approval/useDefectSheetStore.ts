...
export const useDefectSheetStore = defineStore({
  id: "useDefectSheetStore",
  state: () => {
    return {
      ...
      stateSelectedFitter: {} as Employee
    }
  },
  getters: {
    DefectSheets: (state) => {
    ...
    SelectedFitter: (state) => {
      return state.stateSelectedFitter;
    }
  },
  actions: {
    ...
    setSelectedDefectFromOtherPage(defectSheet: DefectSheet) {
      this.stateSelectedDefectSheet = defectSheet
      this.stateSelectedWorkOrder = defectSheet.workOrder
    },
    async approveDefectSheetBySupervisor() {
      ...
      try {
        ...
        if (res.data.result.IsError == 'You cannot approve this digital service sheet because another user already approved.') {
    ...
    updateSelectedFitter(employee: Employee) {
      this.stateSelectedFitter = employee
    }
  }
})
