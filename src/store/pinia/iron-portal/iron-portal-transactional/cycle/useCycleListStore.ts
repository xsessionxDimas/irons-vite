...
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-transactional/cycle/ListItem";
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-transactional/cycle/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-transactional/cycle/LookupItem";
...
export const useCycleListStore = defineStore({
  id: "cycleList",
  state: () => {
    return {
      stateFilterData: {
      ...
      stateHoCycleTargetOption: [] as Option[],
      stateSiteCycleTargetOption: [] as Option[],
    }
  },
  getters: {
    pageName: (state) => {
    ...
    hoCycleTargetOption: (state) => {
      return state.stateHoCycleTargetOption;
    },
    siteCycleTargetOption: (state) => {
      return state.stateHoCycleTargetOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        ...
        hoCycleTarget: this.stateFilterData.hoCycleTarget,
        hoCycleTargetTo: this.stateFilterData.hoCycleTargetTo,
        siteCycleTarget: this.stateFilterData.siteCycleTarget,
        siteCycleTargetTo: this.stateFilterData.siteCycleTargetTo,
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "cycle", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateModelOption = mapOptionWithThreeLabelFromLookupApi(response.data.result.content.equipmentModel, "equipmentModel", "brand", "equipmentModelDescription");
        ...
        this.stateHoCycleTargetOption = mapOption(response.data.result.content.hoCycleTarget);
        this.stateSiteCycleTargetOption = mapOption(response.data.result.content.siteCycleTarget);
      } catch (error: any) {
    ...
    setHoCycleTarget(value: string) {
      this.stateFilterData.hoCycleTarget = value;
    },
    setHoCycleTargetTo(value: string) {
      this.stateFilterData.hoCycleTargetTo = value;
    },
    setSiteCycleTarget(value: string) {
      this.stateFilterData.siteCycleTarget = value;
    },
    setSiteCycleTargetTo(value: string) {
      this.stateFilterData.siteCycleTargetTo = value;
    },
    ...
    async resetFilter() {
      ...
      this.stateFilterData.hoCycleTarget = "";
      this.stateFilterData.hoCycleTargetTo = "";
      this.stateFilterData.siteCycleTarget = "";
      this.stateFilterData.siteCycleTargetTo = "";
      ...
      const checkEquipment = this.stateLastUsedFilterData.equipmentNumber !== "";
      const checkEquipmentTo = this.stateLastUsedFilterData.equipmentNumberTo !== "";
      ...
      const checkHoCycleTarget = this.stateLastUsedFilterData.hoCycleTarget !== "";
      const checkHoCycleTargetTo = this.stateLastUsedFilterData.hoCycleTargetTo !== "";
      const checkSiteCycleTarget = this.stateLastUsedFilterData.siteCycleTarget !== "";
      const checkSiteCycleTargetTo = this.stateLastUsedFilterData.siteCycleTargetTo !== "";
      ...
      if (
        checkSite || checkSiteTo || checkEquipmentModel || checkEquipmentModelTo ||
