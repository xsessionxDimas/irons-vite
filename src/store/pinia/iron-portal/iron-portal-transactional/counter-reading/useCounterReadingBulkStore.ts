...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-transactional/counter-reading/BulkItem";
import {
  ValidatedItem
...
export const useCounterReadingBulkStore = defineStore({
  id: "counterReadingBulk",
  state: () => {
    return {
  ...
  getters: {
    ...
    errorBulkMsg: (state) => {
      return state.stateErrorBulkMsg;
    }
  },
  actions: {
      ...
      try {
        ...
        if (response.data.statusCode == 200) {
          if (!response.data.result.isError) {
            ...
            // check if All submitted data is not valid
            // this.stateIsError = _.every(this.stateValidatedData, {
            //   'isValid': false
            // });
            if (_.every(this.stateValidatedData, {
              'validationReason': "Counter Reading already exists"
            })) {
              this.stateIsError = true;
              this.stateErrorBulkMsg = "There is no new data, all data has been uploaded previously.";
            } else if (_.every(this.stateValidatedData, {
              'isValid': false
            })) {
              this.stateIsError = true;
              this.stateErrorBulkMsg = "There is no new data, please check for the remark.";
            }

            console.log(this.stateIsError);
            this.setPage(1);
          } else {
            this.stateIsError = true;
