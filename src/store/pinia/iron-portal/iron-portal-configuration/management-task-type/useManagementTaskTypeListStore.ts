...
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/management-task-type/ListItem";
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/management-task-type/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/management-task-type/LookupItem";
...
export const useManagementTaskTypeListStore = defineStore({
  id: "managementTaskTypeList",
  state: () => {
    return {
      stateFilterData: {
        ...initialFilter
      ...
      statePageName: "managementTaskType",
      stateDisplayData: [] as ListItem[],
      statePagination: new PaginationType(),
      stateLoading: false as boolean,
      statePaginationLoading: false as boolean,
      stateTaskTypeCodeOption: [] as Option[],
    }
  },
  getters: {
    pageName: (state) => {
    ...
    taskTypeCodeOption: (state) => {
      return state.stateTaskTypeCodeOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        taskTypeCode: this.stateFilterData.taskTypeCode,
        taskTypeCodeTo: this.stateFilterData.taskTypeCodeTo,
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateTaskTypeCodeOption = mapOptionFromLookupApi(response.data.result.content, "taskTypeCode", "taskTypeDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setTaskTypeCode(value: string) {
      this.stateFilterData.taskTypeCode = value;
    },
    setTaskTypeCodeTo(value: string) {
      this.stateFilterData.taskTypeCodeTo = value;
    },
    setTaskTypeDescription(value: string) {
      this.stateFilterData.taskTypeDescription = value;
    },
    setTaskTypeDescriptionTo(value: string) {
      this.stateFilterData.taskTypeDescriptionTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.taskTypeCode = "";
      this.stateFilterData.taskTypeCodeTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkTaskTypeCode = this.stateLastUsedFilterData.taskTypeCode !== "";
      const checkTaskTypeCodeTo = this.stateLastUsedFilterData.taskTypeCodeTo !== "";
