...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/item/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/item/LookupItem";
...
export const useItemListStore = defineStore({
  id: "itemList",
  state: () => {
    return {
      stateFilterData: {
  ...
  getters: {
    ...
    itemOption: (state) => {
      return state.stateItemOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        item: this.stateFilterData.item,
        itemTo: this.stateFilterData.itemTo,
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "item", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateItemOption = mapOptionFromLookupApi(response.data.result.content, "item", "itemDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setItem(value: string) {
      this.stateFilterData.item = value;
    },
    setItemTo(value: string) {
      this.stateFilterData.itemTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.item = "";
      this.stateFilterData.itemTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkItem = this.stateLastUsedFilterData.item !== "";
      const checkItemTo = this.stateLastUsedFilterData.itemTo !== "";
      ...
      if (checkItem || checkItemTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
