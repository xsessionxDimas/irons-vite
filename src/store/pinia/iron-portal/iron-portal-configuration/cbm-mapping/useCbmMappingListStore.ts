...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/cbm-mapping/FilterData";
import {
  LookupItem
...
export const useCbmMappingListStore = defineStore({
  id: "cbmMappingList",
  state: () => {
    return {
      stateFilterData: {
  ...
  actions: {
    async getData(isPageRefresh = true) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "cbm_mapping", new URLSearchParams(params).toString());
