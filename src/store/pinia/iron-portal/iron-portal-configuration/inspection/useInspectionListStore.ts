...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/inspection/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/inspection/LookupItem";
...
export const useInspectionListStore = defineStore({
  id: "inspectionList",
  state: () => {
    return {
      stateFilterData: {
      ...
      statePageName: "inspection",
      stateDisplayData: [] as ListItem[],
      statePagination: new PaginationType(),
      stateLoading: false as boolean,
      statePaginationLoading: false as boolean,
      stateInspectionOption: [] as Option[],
    }
  },
  getters: {
    pageName: (state) => {
    ...
    inspectionOption: (state) => {
      return state.stateInspectionOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        inspection: this.stateFilterData.inspection,
        inspectionTo: this.stateFilterData.inspectionTo,
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "inspection", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateInspectionOption = mapOptionFromLookupApi(response.data.result.content, "inspection", "inspectionDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setInspection(value: string) {
      this.stateFilterData.inspection = value;
    },
    setInspectionTo(value: string) {
      this.stateFilterData.inspectionTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.inspection = "";
      this.stateFilterData.inspectionTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkInspection = this.stateLastUsedFilterData.inspection !== "";
      const checkInspectionTo = this.stateLastUsedFilterData.inspectionTo !== "";
      ...
      if (checkInspection || checkInspectionTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
