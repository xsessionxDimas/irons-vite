...
export const useRiskManagementRatingFormStore = defineStore({
  id: "riskManagementRatingForm",
  state: () => {
    return {
      stateFormData: {
        ...initialForm
  ...
  actions: {
    async insertData(createBy: string) {
      try {
        ...
        const formData = {
          managementRatingId: 0,
          ratingParameter: this.stateFormData.ratingParameter,
          ratingCode: this.stateFormData.ratingCode,
          ratingDescription: this.stateFormData.ratingDescription,
    ...
    async updateData(updateBy: string) {
      try {
        ...
        const formData = {
          managementRatingId: this.stateFormData.managementRatingId,
          ratingParameter: this.stateFormData.ratingParameter,
          ratingCode: this.stateFormData.ratingCode,
          ratingDescription: this.stateFormData.ratingDescription,
          startDate: formatDateForPostData(normalizeDate(new Date(this.stateFormData.startDate))),
    ...
    setFormData(data: UpsertItem) {
      this.stateFormData.managementRatingId = data.managementRatingId;
      this.stateFormData.ratingParameter = data.ratingParameter;
      this.stateFormData.ratingCode = data.ratingCode;
      this.stateFormData.ratingDescription = data.ratingDescription;
