import {
  BULK_API_URL,
  UPLOAD_API_URL,
  BULK_API_URL_SPECIFIC,
  UPLOAD_API_URL_SPECIFIC,
  DOWNLOAD_TEMPLATE
} from "./urls";
...
export const useCbmThresholdBulkStore = defineStore({
  id: "cbmThreshold",
  state: () => {
    return {
  ...
  actions: {
    async bulkInsert(userAccount: string, isSpecific = false) {
      const params = {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<BulkItem>> = await ApiService.post(`${isSpecific ? BULK_API_URL_SPECIFIC : BULK_API_URL}?${new URLSearchParams(params).toString()}`, this.stateBulkData as AxiosRequestConfig);
        ...
        this.stateIsError = error.response.data.result.isError;
    ...
    async upload({ userAccount, excelFile }, isSpecific = false): Promise<void> {
      console.log(isSpecific ? UPLOAD_API_URL_SPECIFIC : UPLOAD_API_URL)
      const params = {
        userAccount: userAccount,
        ver: "v1"
      }
      const url = `${isSpecific ? UPLOAD_API_URL_SPECIFIC : UPLOAD_API_URL}?${new URLSearchParams(params).toString()}`;
      try {
    ...
    async downloadTemplate(type: string) {
      const params = {
        cbmThresholdType: type,
        ver: "v1"
      };
      try {
        const response: AxiosResponse<Blob> = await ApiService.getBlob(`${DOWNLOAD_TEMPLATE}?${new URLSearchParams(params).toString()}`, "");
