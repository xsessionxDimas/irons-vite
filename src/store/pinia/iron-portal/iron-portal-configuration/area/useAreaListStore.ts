...
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/area/ListItem";
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/area/FilterData";
import {
  LookupItem
...
export const useAreaListStore = defineStore({
  id: "areaList",
  state: () => {
    return {
      stateFilterData: {
  ...
  actions: {
    async getData(isPageRefresh = true) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "area", new URLSearchParams(params).toString());
    ...
    async resetFilter() {
      ...
      if (checkArea || checkAreaTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
