...
import {
  UpsertItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/area/UpsertItem";
import {
  ListItem
...
export const useAreaFormStore = defineStore({
  id: "areaForm",
  state: () => {
    return {
      stateFormData: {
  ...
  actions: {
    async insertData(createBy: string) {
      try {
        ...
        const formData = {
          areaId: this.stateFormData.areaId,
    ...
    setFormData(data: ListItem) {
      this.stateFormData.areaId = data.AreaId;
