...
import {
  UpsertItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/damage-category/UpsertItem";
import {
  ListItem
...
const initialForm = {
  damageCategoryId: 0,
  damageCategory: "",
  damageCategoryDescription: "",
  startDate: normalizeDate(new Date()),
  endDate: normalizeDate(new Date("12/31/9999")),
};

export const useDamageCategoryFormStore = defineStore({
  id: "damageCategoryForm",
  state: () => {
    return {
      stateFormData: {
        ...initialForm
  ...
  actions: {
    async insertData(createBy: string) {
      try {
        ...
        const formData = {
          damageCategoryId: this.stateFormData.damageCategoryId,
          damageCategory: this.stateFormData.damageCategory,
          damageCategoryDescription: this.stateFormData.damageCategoryDescription,
    ...
    setFormData(data: ListItem) {
      this.stateFormData.damageCategoryId = data.DamageCategoryId;
      this.stateFormData.damageCategory = data.DamageCategory;
      this.stateFormData.damageCategoryDescription = data.DamageCategoryDescription;
