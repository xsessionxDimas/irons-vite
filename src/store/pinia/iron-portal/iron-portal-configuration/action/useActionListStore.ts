...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/action/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/action/LookupItem";
...
export const useActionListStore = defineStore({
  id: "actionList",
  state: () => {
    return {
      stateFilterData: {
      ...
      statePageName: "action",
      stateDisplayData: [] as ListItem[],
      statePagination: new PaginationType(),
      stateLoading: false as boolean,
      statePaginationLoading: false as boolean,
      stateActionOption: [] as Option[],
    }
  },
  getters: {
    pageName: (state) => {
    ...
    actionOption: (state) => {
      return state.stateActionOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        action: this.stateFilterData.action,
        actionTo: this.stateFilterData.actionTo,
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "action", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateActionOption = mapOptionFromLookupApi(response.data.result.content, "action", "actionDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setAction(value: string) {
      this.stateFilterData.action = value;
    },
    setActionTo(value: string) {
      this.stateFilterData.actionTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.action = "";
      this.stateFilterData.actionTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkAction = this.stateLastUsedFilterData.action !== "";
      const checkActionTo = this.stateLastUsedFilterData.actionTo !== "";
      ...
      if (checkAction || checkActionTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
