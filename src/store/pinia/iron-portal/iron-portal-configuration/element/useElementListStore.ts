...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/element/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/element/LookupItem";
...
export const useElementListStore = defineStore({
  id: "elementList",
  state: () => {
    return {
      stateFilterData: {
  ...
  actions: {
    async getData(isPageRefresh = true) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "element", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateElementOption = mapOptionFromLookupApi(response.data.result.content, "element", "elementDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    async resetFilter() {
      ...
      if (checkElement || checkElementTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
