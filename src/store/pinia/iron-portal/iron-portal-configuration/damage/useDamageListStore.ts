...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/damage/FilterData";
import {
  LookupItem
...
export const useDamageListStore = defineStore({
  id: "damageList",
  state: () => {
    return {
      stateFilterData: {
  ...
  actions: {
    async getData(isPageRefresh = true) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "damage", new URLSearchParams(params).toString());
    ...
    async resetFilter() {
      ...
      if (checkDamage || checkDamageTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
