...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/week/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/week/LookupItem";
...
export const useWeekListStore = defineStore({
  id: "weekList",
  state: () => {
    return {
      stateFilterData: {
  ...
  getters: {
    ...
    weekOption: (state) => {
      return state.stateWeekOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        week: this.stateFilterData.week,
        weekTo: this.stateFilterData.weekTo,
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "week", new URLSearchParams(params).toString());
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateWeekOption = mapOptionFromLookupApi(response.data.result.content, "week", "weekDescription");
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setWeek(value: string) {
      this.stateFilterData.week = value;
    },
    setWeekTo(value: string) {
      this.stateFilterData.weekTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.week = "";
      this.stateFilterData.weekTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkWeek = this.stateLastUsedFilterData.week !== "";
      const checkWeekTo = this.stateLastUsedFilterData.weekTo !== "";
      ...
      if (checkWeek || checkWeekTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
