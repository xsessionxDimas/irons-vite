...
export const useRiskRankMaterialFormStore = defineStore({
  id: "riskRankMaterialForm",
  state: () => {
    return {
      stateFormData: {
        ...initialForm
  ...
  actions: {
    async insertData(createBy: string) {
      try {
        ...
        const formData = {
          riskRankIntervalId: 0,
          maxRiskRank: this.formData.maxRiskRank,
          revisedRisk: this.formData.revisedRisk,
          overallRisk: this.formData.overallRisk,
          failureTiming: this.formData.failureTiming,
          systemImpact: this.formData.systemImpact,
          opsImpact: this.formData.opsImpact,
          supplyRisk: this.formData.supplyRisk,
          failureCost: this.formData.failureCost,
    ...
    async updateData(updateBy: string) {
      try {
        ...
        const formData = {
          riskRankIntervalId: this.formData.riskRankIntervalId,
          maxRiskRank: this.formData.maxRiskRank,
          revisedRisk: this.formData.revisedRisk,
          overallRisk: this.formData.overallRisk,
          failureTiming: this.formData.failureTiming,
    ...
    setFormData(data: ListItem) {
      this.stateFormData.riskRankIntervalId = data.RiskRankIntervalId;
      this.stateFormData.maxRiskRank = data.MaxRiskRank;
      this.stateFormData.revisedRisk = data.RevisedRisk;
