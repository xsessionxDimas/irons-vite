...
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/code-group/FilterData";
import {
  LookupItem
...
export const useCodeGroupListStore = defineStore({
  id: "codeGroupList",
  state: () => {
    return {
      stateFilterData: {
  ...
  actions: {
    async getData(isPageRefresh = true) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<ListItem>> = await ApiService.get(GET_API_URL, "code_group", new URLSearchParams(params).toString());
    ...
    async resetFilter() {
      ...
      if (checkCodeGroup || checkCodeGroupTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
