...
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/revise-user/ListItem";
import {
  FilterData
} from "@/core/types/entities/iron-portal/iron-portal-configuration/revise-user/FilterData";
import {
  LookupItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/revise-user/LookupItem";
...
export const useReviseUserListStore = defineStore({
  id: "reviseUserList",
  state: () => {
    return {
      stateFilterData: {
      ...
      statePageName: "reviseUser",
      stateDisplayData: [] as ListItem[],
      statePagination: new PaginationType(),
      stateLoading: false as boolean,
      statePaginationLoading: false as boolean,
      stateReviseUserOption: [] as Option[],
    }
  },
  getters: {
    pageName: (state) => {
    ...
    reviseUserOption: (state) => {
      return state.stateReviseUserOption;
    },
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        reviseUser: this.stateFilterData.reviseUser,
        reviseUserTo: this.stateFilterData.reviseUserTo,
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateReviseUserOption = mapOption(response.data.result.content.reviseUser.employeeName);
      } catch (error: any) {
        if (error.response.data.statusCode == 401) {
          const authStore = useAuthenticationStore();
          authStore.setLoggedIn(false);
    ...
    setReviseUser(value: string) {
      this.stateFilterData.reviseUser = value;
    },
    setReviseUserTo(value: string) {
      this.stateFilterData.reviseUserTo = value;
    },
    ...
    async resetFilter() {
      this.stateFilterData.reviseUser = "";
      this.stateFilterData.reviseUserTo = "";
      this.stateFilterData.startDate = "";
      this.stateFilterData.startDateTo = "";
      this.stateFilterData.endDate = "";
      this.stateFilterData.endDateTo = "";
      const checkReviseUser = this.stateLastUsedFilterData.reviseUser !== "";
      const checkReviseUserTo = this.stateLastUsedFilterData.reviseUserTo !== "";
      ...
      if (checkReviseUser || checkReviseUserTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo) {
        await this.getData()
      }
    },
    resetState() {
