export const CRUD_API_URL = `${import.meta.env.VITE_APP_BASE_URL_DIGITAL}/${import.meta.env.VITE_APP_API_AM_ADM}/api/master_data/equipment_model`
export const BULK_API_URL = `${CRUD_API_URL}/bulk`
export const UPLOAD_API_URL = `${CRUD_API_URL}/validate_upload`
export const EXPORT_API_URL = `${CRUD_API_URL}/export`
export const LOOKUP_API_URL = `${CRUD_API_URL}/lookup`
export const LOOKUP_TRANSACTION_API_URL = `${CRUD_API_URL}/lookup_transaction`
export const UPDATE_API_URL = `${CRUD_API_URL}/update`
export const BRAND_LOOKUP_URL = `${CRUD_API_URL}/lookup_brand`
