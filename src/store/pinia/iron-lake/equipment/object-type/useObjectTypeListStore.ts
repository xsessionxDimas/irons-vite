...
import {
  ListItem
} from "@/core/types/entities/iron-lake/equipment/object-type/ListItem";
import {
  FilterData
...
export const useObjectTypeListStore = defineStore({
  ...
  state: () => {
    return {
      ...
      statePageName: "objectType",
      stateDisplayData: [] as ListItem[],
      statePagination: new PaginationType(),
      stateLoading: false as boolean,
      statePaginationLoading: false as boolean,
      stateObjectTypeOption: [] as Option[],
      stateObjectTypeDescOption: [] as Option[]
    }
  },
  getters: {
    pageName: (state) => {
    ...
    objectTypeDescOption: (state) => {
      return state.stateObjectTypeDescOption;
    }
  },
  actions: {
    async getData(isPageRefresh = true) {
      const params = {
        ...
        ObjectTypeDescription: this.stateFilterData.ObjectTypeDescription,
    ...
    async getLookup() {
      ...
      try {
        ...
        this.stateObjectTypeDescOption = mapOption(response.data.result.content.objectTypeDescription);
      } catch (error) {
        console.log(error)
      }
    },
    ...
    setObjectTypeDescription(value: string) {
      this.stateFilterData.ObjectTypeDescription = value;
    },
    async resetFilter() {
      this.stateFilterData.ObjectType = ""
      this.stateFilterData.ObjectTypeDescription = ""
      const checkObjectType = this.stateLastUsedFilterData.ObjectType !== ""
      const checkObjectTypeDesc = this.stateLastUsedFilterData.ObjectTypeDescription !== ""
      if (checkObjectType || checkObjectTypeDesc) {
        await this.getData()
      }
    },
    resetState() {
