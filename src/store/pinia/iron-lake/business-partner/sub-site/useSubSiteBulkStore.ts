...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/business-partner/sub-site/BulkItem";
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/business-partner/sub-site/ValidatedItem";
import PaginationType from "@/core/types/misc/Pagination";
import { some } from "lodash";
import { dynamicSort } from "@/core/helpers/table-sort";
import { SortParam } from "@/core/types/misc/SortParam";

export const useSubSiteBulkStore = defineStore({
  id: "subSiteBulk",
  state: () => {
    return {
  ...
  actions: {
    async bulkInsert(userAccount: string) {
      ...
      try {
        ...
        const response: AxiosResponse<ApiResponse<BulkItem>> = await ApiService.post(`${apiUrls.uploadBulk}?${new URLSearchParams(params).toString()}`, this.stateBulkData as AxiosRequestConfig);
    ...
    async upload({ userAccount, excelFile }): Promise<void> {
      ...
      const url = `${apiUrls.validateUpload}?${new URLSearchParams(params).toString()}`;
      try {
        ...
        if (response.data.statusCode == 200) {
          ...
          this.stateIsError = response.data.result.isError ?? false;
          this.stateError = response.data.result?.message;
        }
      } catch (error) {
        /* log mechanism goes here since its unhandled */
        this.stateIsError = true;
        this.stateError = "Unhandled exception occured please try again";
      }
