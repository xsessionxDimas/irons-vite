import { BULK_API_URL, IRONLAKE_CRUD_API_URL, UPLOAD_API_URL } from "./urls";
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/business-partner/site/BulkItem";
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/business-partner/site/ValidatedItem";
...
export const useSiteBulkStore = defineStore({
  id: "siteBulk",
  state: () => {
    return {
  ...
  actions: {
    async bulkUpload() {
      ...
      try {
        const response: AxiosResponse<SingleApiResponse<BulkResponse>> = await ApiService.post(`${IRONLAKE_CRUD_API_URL}/bulk?ver=v1`, uploadDataPayload as AxiosRequestConfig)
        if (response.data.statusCode == 200) {
          if (!response.data.result.isError) {
            const content = response.data.result.content;
            const totalUploaded = content.totalData;
            const invalidRecord = content.siteList.filter((item) => {
