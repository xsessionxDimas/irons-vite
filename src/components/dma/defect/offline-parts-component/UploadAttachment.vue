...
const onDrop = async (acceptFiles) => {
  ...
  if (!isFilePassValidation) {
    ...
    const taskFile: QueueFileTask = {
      ...
      filename: fileName,
      originalFilename: acceptFiles[0].name,
      blob: blob,
      syncStatus: "Pending",
    ...
    if (!isOfflineOrSlowInternetConnection()) {
      promise = await attachmentStore.uploadAttachment(payload, authStore.user.Email, taskFile)
    } else {
      promise = await addRecord(db, 'pendingTaskFile', taskFile);
    }
