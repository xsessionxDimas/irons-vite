<template>
  <el-dialog
    ...
    <div
      ...
      <camera
        ...
        <template v-if="isCameraReady">
          <div
            ...
            <div v-else class="d-flex flex-column justify-content-center align-items-center camera-tools-container" style="position: relative;">
      ...
      <template v-if="cameraStore.isVisible && iscameraPreviewing">
        <div class="preview-container">
          <div
            ...
            <vue-drawing-canvas
              ...
              saveAs="png"
              :lock="isLockedDraw"
              :output-width="resizedImgOutputWidth"
              :output-height="resizedImgOutputHeight"
            />
          </div>
          <div
...
const backToCapturing = () => {
  ...
  canvasScreenWidth.value = window.innerWidth;
  canvasScreenHeight.value = window.innerHeight;
...
const handleUpdateCanvasHeightAndWidth = () => {
  console.log("screen available width", window.innerWidth)
  console.log("screen available height", window.innerHeight)
  console.log("picture width on as param", takenPicWidth)
  console.log("picture height on as param", takenPicHeight)
...
const drawBgImage = (imageSrc: string, isTakePhoto) => {
  ...
  image.onload = () => {
    if (window.screen.height > window.screen.width) {
      ...
      console.log("image is taken by camera", isTakePhoto)
      console.log("original take photo result's height", image.height);
      console.log("original take photo result's width", image.width);
  ...
  if (promise) {
    Promise.all([promise]).then(() => {
      ...
      if (cameraStore.IsFromActionDelete) {
        cameraStore.definePhoto(blobImage.value as Blob, image);
      } else {
        cameraStore.addPhoto(blobImage.value as Blob, image);
      }
    });
  }
  loading.close();
...
const resizedImgOutputWidth = computed(() => {
  return (canvasScreenWidth.value * resizePercentage) / 100;
});
const resizedImgOutputHeight = computed(() => {
  return (canvasScreenHeight.value * resizePercentage) / 100;
});
