<template>
  <el-dialog
    ...
    <div
      v-loading.fullscreen.lock="blocker"
    ...
    >
      ...
      <template v-if="isPreview">
        <div class="preview-container">
          <div
            class="d-flex justify-content-center align-items-center selected-container"
          >
            <vue-drawing-canvas
              ref="VueCanvasDrawing"
              v-model:image="drawImage"
              ...
              :initial-image="savedDraw"
              saveAs="png"
              :lock="isLockedDraw"
              :output-width="canvasScreenWidth"
              :output-height="canvasScreenHeight"
            />
          </div>
          <!-- New Implementation -->
          <div
            ...
            :style="{
              backgroundColor: isLockedDraw ? '' : 'black',
              bottom: isLockedDraw ? '16px' : '0',
            }"
          >
            <div
              v-if="isLockedDraw"
            ...
            <div v-else id="config-wrapper">
              ...
              :class="boxConfig">
                ...
                <section class="flex-fill d-flex flex-row justify-content-between align-items-center">
                  <div class="d-flex gap-4">
                    <p
                      ...
                      @click.prevent="VueCanvasDrawing.undo()">
                      <svg
                        width="16"
                        height="17"
                        viewBox="0 0 16 17"
                    ...
                    <p
                      class="tools-item tools-active d-flex align-items-center m-0 px-3 py-2"
                      @click.prevent="VueCanvasDrawing.redo()">
                      <svg
                        width="16"
                        height="17"
                        viewBox="0 0 16 17"
                  ...
                  <div class="d-flex gap-4">
                    <p
                      ...
                      :class="!isEraser ? 'tools-active' : ''"
                      @click.prevent="isEraser = false">
                      <svg
                        width="16"
                        height="17"
                        xmlns="http://www.w3.org/2000/svg"
                ...
                <section class="flex-fill d-flex flex-row align-items-center">
                  ...
                  <el-slider v-model="lineWidth" class="flex-fill" :min="1" />
                  <p class="m-0 ms-4">{{ lineWidth }}</p>
                </section>
...
const currentDeviceID = computed(() => {
  return cam.value?.currentDeviceID();
});
...
const changeCamera = async () => {
  ...
  if (devices) {
    /* default always start to zero */
    let newDeviceId = devices[0].deviceId;
    if (currentDeviceID.value && currentDeviceID.value != newDeviceId) {
      cam.value?.changeCamera(newDeviceId);
    } else {
      newDeviceId = devices[1].deviceId;
      cam.value?.changeCamera(newDeviceId);
    }
  }
};
...
const closeCamera = () => {
  selectedCameraTimer.value = 0;
  isCameraErrorPopUpShown.value = false
  emits("onCloseCamera", true);
};
const backToCapturing = () => {
  blocker.value = true;
  isTakePicture.value = false
  selectedCameraTimer.value = 0;
  emits("onBackToCapture", null);
};
...
const handleUpdateCanvasHeightAndWidth = () => {
  ...
  canvasScreenWidth.value = Math.floor(width);
  canvasScreenHeight.value = Math.floor(height);
}
...
const drawBgImage = (imageSrc: string, isTakePhoto) => {
  ...
  image.onload = () => {
    ...
    VueCanvasDrawing.value.setBackground();
  };
};
...
const SnapshotSave = () => {
  doneFreeDraw()
  // check available strokes a.k.a edited images strokes
  emits("onSnapshotSave", props.imageDescription, blobImage.value as Blob);
  isTakePicture.value = false
};
...
const openDescriptionForm = () => {
  emits("onAddDescription");
};

// Edit with draw image module
const VueCanvasDrawing = ref();
const isLockedDraw = ref(true);
const drawImage = ref("");
const isEraser = ref(false);
const lineWidth = ref(10);
const lineColor = ref("rgba(242, 24, 24, 1)");
const savedDraw = ref([]);
const canvasScreenWidth = ref(window.innerWidth);
const canvasScreenHeight = ref(window.innerHeight);

const handleDrawImage = () => {
  isLockedDraw.value = false;
};
...
const cancelFreeDraw = () => {
  isLockedDraw.value = true;
  VueCanvasDrawing.value.reset();
};

const doneFreeDraw = () => {
  isLockedDraw.value = true;
  blobImage.value = b64toBlob(drawImage.value.split(",")[1], "image/png");
};
</script>
