  ...
  <div :class="[widthStyle, potraitWidthStyle, paddingBottomCondition]" class="px-2 pt-2 input-eform-status" :style="style">
    ...
    <template v-if="loadingIndicator">
      <div class="d-flex align-items-center">
        ...
        <span class="timestamp-task">Calculating...</span>
      </div>
    </template>
...
import OilNotInRange from './OilNotInRange.vue';
import {
  useCameraStore
...
const disabledCondition = computed(() => {
  ...
  return cond || props.task!.isShowAdjustmentRow || dynamicParamRatingUomNotSelected || disabledByOtherItem.value || scHeightInputNDropdownYes || isDisabledByKeyDropdownToolGroup.value
})
...
const handleCalculateAvg = async (val) => {
  ...
  if (isArray(arrKey)) {
    for (const e of arrKey) {
      const num = await store.getValueFromItemKey(e)
...
const readOnly = computed(() => {
  ...
  if (taskPropsItem.value?.updatedBy) {
    ...
    if (itemRef.value?.updatedBy) {
      if (store.employee.id != (itemRef.value.updatedBy as Audit).id) {
    ...
    await store.getValueFromItemKey(itemRef.value.mappingKeyId! as string).then(async (validationVal) => {
      if (validationVal != '') {
        ...
        if (checkMin && checkMax) {
          ...
          validation.value = `Range refill capacity is between ${minVal} until ${maxVal}`
          loadingCommentIndicator.value = false
