  ...
  <div class="condition-modified my-5" v-if="props.isModifying">
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        >
          <th scope="row" class="ps-3 align-middle" v-text="questionRow.items[1].value" />
          <th scope="row" class="align-middle" v-text="questionRow.items[2].value" />
          ...
          <th
            scope="row"
            class="align-middle"
            v-text="modifiedObject.value[qRowIndex].uom"
          />
          <th scope="row" class="align-middle">
            <div
          ...
          <th scope="row" class="align-middle">
            <SmallCameraModify
              :itemkey="`avg-img-${qRowIndex}`"
              @click="last_clicked_cam_index = qRowIndex"
              :images="overwriteStore.modifiedObjectAvg.value[qRowIndex].images"
    ...
    <table class="table table-bordered">
      ...
      <thead>
        <tr style="background: #f4f6f8">
          ...
          <th scope="col" class="table-header px-4">Rating</th>
      ...
      <tbody
        v-if="
          ...
          detailCbm.historyModified.detail?.history.length > 0
        "
      >
        <template
        ...
        >
          <tr>
            <td class="p-4 align-middle" v-text="getTaskNo(item.items[0].value)" />
            ...
            <td class="p-4 align-middle input-eform-status">
              <el-input
                class="input-overwrite-cbm"
                :class="dropDownColor(item.items[5].value)"
      ...
      <tbody v-else>
        <tr>
          <td colspan="6" class="table-header text-center py-8">No Data Recorded</td>
        </tr>
      </tbody>
    </table>
  </div>
...
const getTaskNo = (taskNo) => {
  const numberFromHistory = detailCbm.value.historyModifiedDefault[0].description.split(';')[0];
  if (taskNo == '') {
...
const countAverage = () => {
  ...
  if (blankValueIndex != -1 && blankValueIndex != lastIndex) {
    overwriteStore.stateIsError = true;
    return;
  }
  let sum = 0;
  for (let i = 0; i < lastIndex; i++) {
    if (isNaN(Number(modifiedObject.value.value[i].measurementValue))) {
      overwriteStore.setModifiedData(lastIndex, "measurementValue", "");
      overwriteStore.setModifiedData(lastIndex, "measurementRating", "");
      return;
    }
  ...
  overwriteStore.setModifiedData(
    lastIndex,
    "measurementValue",
    parseFloat((sum / (modifiedObject.value.value.length - 1)).toString())
      .toFixed(2)
      .toString()
  );
...
const changeEventToValue = (qRowIndex: number) => {
  overwriteStore.resetErrors();

  if (
    overwriteStore.stateInputErrorList.length !=
    detailCbm.value.historyModifiedDefault.length
  ) {
    overwriteStore.initErrorInputCbmAverage();
  }
...
const checkMeasurementValueInputValid = (qRowIndex) => {
  return maximumFiveDigitWithTwoFraction(
    modifiedObject.value.value[qRowIndex].measurementValue
  );
};
...
const handleModifiedImage = (images) => {
  ...
  overwriteStore.setImagePerRowAverage(index, images);
};
...
const isPhotoMandatory = (qRowIndex) => {
  const customValidation =
    detailCbm.value.historyModifiedDefault[qRowIndex].items[3].customValidation;
...
const openCameraIfMandatory = (qRowIndex) => {
  if (
    modifiedObject.value.value[qRowIndex].images.length < 1 &&
    isPhotoMandatory(qRowIndex)
  ) {
