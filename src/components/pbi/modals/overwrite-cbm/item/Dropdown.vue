...
const readOnly = computed(() => {
  ...
  // for other fitter (still in progress)
  // if (!checkSameFitter.value && itemRef.value.value != '') {
  //   if ((itemRef.value.value === '3' && props.task?.category === 'NORMAL') || (itemRef.value.value == '3' && itemRef.value.categoryItemType == 'resultStatus') || (itemRef.value.value === '4' && props.task?.category === 'CRACK')) {
  //     // if NA
  //     sameFitter = true
  //   } else if (itemRef.value.value === '1') {
  //     // if ok system complete/No Crack Identified
  //     const okLabel = options.value.find((val) => {
  //       return val.value == "1"
  //     })
  //     if (okLabel?.label == "1 - Complete") {
  //       sameFitter = false
  //     }
  //   } else if (CBM.includes(itemRef.value.value as string) && taskPropsItem.value?.category.includes("CBM")) {
  //     // if cbm
  //     sameFitter = true
  //   } else {
  //     sameFitter = true
  ...
  if (normalCategory || crackCategory || cbmCategory) {
    sameFitter = true
  }

  let defect = false
  // if ((props.task?.category == 'CRACK' && inputValue.value == '2') || (props.task?.category == 'NORMAL' && inputValue.value == '2') || (props.task?.category == 'CRACK' && inputValue.value == '3')) {
  //   defect = true
  // }
  ...
  return !sameFitter || defect || disabledByOtherItem.value || loadingIndicator.value || isSCAdjustmentYes
});
...
const readOnlyMultipleDefect = computed(() => {
  ...
  return !sameFitter || disabledByOtherItem.value || loadingIndicator.value
});
