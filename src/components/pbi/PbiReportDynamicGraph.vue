...
import {
  getAllDataFromPbiTableOneColumn,
  getAllOptionsFromPbiSlicer
} from "./utils"

const { configTargetY1, configTargetY2, defaultY1, defaultY2, getFilterY } = dynamicGraphHierarchy();
...
// List of filter for Dynamic Graph
    ...
    if (typeof pbiConfig.value !== "undefined") {
      ...
      report.on("loaded", async () => {
        ...
        const page = pages.find((p) => {
          return p.displayName === "Component_Detail_analysis" // get tab Component_Detail_analysis
        });
        ...
        if (loggedInUserLocation && loggedInUserLocation !== "AU02") {
          ...
          embedContainer = document.getElementById("dynamicGraphContainer")!;
        ...
        report.on("dataSelected", async (event) => {
          ...
          if (data?.visual?.title === "component_slicer") {
            setTimeout(async () => {
              ...
              let slicerY2 = visuals.filter((visual) => {
                return visual.type === "slicer" && visual.title === "Y2"
              })[0];
              ...
              let slicerY2Obj = {
                filters: [
                  {
                    $schema: "http://powerbi.com/product/schema#hierarchy",
                    target: configTargetY2,
                    filterType: 9,
                    hierarchyData: defaultY2
                  }
                ],
              };
              ...
              if (dataValue?.length > 0) {
                ...
                let arrFilterY2 = [] as any

                let sampleTypeTable = visuals.filter((visual) => {
                  return visual.title === "table_sample"
                })[0];
                let exportedTableData = await sampleTypeTable.exportData()
                const tableData = getAllDataFromPbiTableOneColumn(exportedTableData ? exportedTableData.data : "");
              ...
              try {
                await slicerY2.setSlicerState(slicerY2Obj);
                console.log("slicer Y2 successfully set")
              } catch (error) {
                console.error("slicer Y2 error", error)
              }
            }, 1500)
          }

          // If it's slicer event, then set session storage
          if (event.detail.visual.type == "slicer" && event.detail.visual.title != "Y1" && event.detail.visual.title != "Y2") {
            let slicer = visuals.filter((visual) => {
        ...
        columnAndTableList.forEach(async (e) => {
          ...
          if (arrayValue && arrayValue.length > 0) {
            const slicerObj = generateSlicerFilterObject(e.table, e.column, e.schema, e.filterType, arrayValue)
            // Set the slicer state which contains the slicer filters.
            try {
              await slicer.setSlicerState(slicerObj);
              console.log("success set slicer " + e.slicerTitle)
              console.log(slicerObj)
            } catch (error) {
              console.log(error)
            }

            // IF FOR SET HEIRARCY ON LOAD IF HAVE COMPONENT ON SESSION STORAGE
            if (e.slicerTitle === 'component_slicer') {
              // Set the Y1 state which contains the slicer filters.
              let slicerY1 = visuals.filter((visual) => {
                return visual.type === "slicer" && visual.title === "Y1"
              })[0];
        ...
        // ==============================================================
        // START :: SET SLICER : DATE                                  ==
        // ==============================================================

        // Set slicer Start Date to day 1 of 6 months based on default End Date Slicer
        ...
        const filter = {
          ...
          target: {
            table: "vw_f_pm_ehms_health_data_temp",
            column: "sample_date"
          },
          filterType: pbi.models.FilterType.Advanced,
          logicalOperator: "And",
        ...
        try {
          ...
          let slicer = visuals.filter(function (visual) {
            return visual.type === "slicer" && visual.name === "5e45efedbb747986d411";
          })[0];

          // Set the slicer state which contains the slicer filters.
          await slicer.setSlicerState({ filters: [filter] });
          console.log("Date slicer was set.");
          const after = await slicer.getSlicerState()
          console.log(after)
        } catch (errors) {
          console.log(errors);
        }
...
const setFilterToSessionStorage = async (slicerTitle: string, operator: string, arrayOfNewValues: string[], allOptions: string[]) => {
  ...
  console.log(newSessionStorageValue, slicerTitle)
...
const generateSlicerFilterObject = (table: string, column: string, schema: string, filterType: number, arrayValue: string[]) => {
  if (schema == "basic") {
    return {
      filters: [
        {
          $schema: "http://powerbi.com/product/schema#${schema}",
          operator: "In",
          values: arrayValue,
          target: {
            column: column,
  ...
  } else if (schema == "advanced") {
    const slicerFilterObj = {
      filters: [
        {
          $schema: "http://powerbi.com/product/schema#${schema}",
          logicalOperator: "And",
    ...
    arrayValue.forEach((e) => {
      slicerFilterObj.filters[0].conditions.push({
        operator: "Is",
        value: e
      })
    })
    return slicerFilterObj
  }
  return {}
}
// end: METHODS ===============================================
