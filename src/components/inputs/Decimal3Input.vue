<template>
  ...
  <label v-if="label" class="form-label fs-6 fw-bolder text-dark">{{ label }}</label>
    <el-form-item>
      <el-input
        v-model="inputValue"
...
/**
 * @description Decimal input 3 angka belakang koma
 */
import {
  computed,
  defineProps,
...
const validate = (e) => {
  let t = e;
  inputValue.value = t.indexOf('.') >= 0 ? t.substr(0, t.indexOf('.')) + t.substr(t.indexOf('.'), 4) : t;
}
...
const parseDecimal = (value) => {
  ...
  if (c === undefined) c = '000';
  else if (c.length === 1) c += '00';
  else if (c.length === 2) c += '0';
  else c = c.toString().substring(0, 3);
...
const removeDecimal = () => {
  ...
  if (inputValue.value === '0' || inputValue.value === '0.00' || inputValue.value === '0.000') {
