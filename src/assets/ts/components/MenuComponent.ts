    ...
    if (items && items.length > 0) {
      for (let i = 0, len = items.length; i < len; i++) {
        ...
        if (menu && menu.getItemSubType(item) === 'dropdown') {
          handleHideMenuOnSkipVar(skip, menu, item)
        }
      }
    }
  }
  ...
  public static initGlobalHandlers = () => {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (menuItems && menuItems.length > 0) {
        handleHideMenuDropdown(menuItems, e)
      }
    })
...
function handleHideMenuOnSkipVar(skip: HTMLElement | undefined, menu: any, item: Element) {
  if (skip) {
    if (
...
function handleHideMenuDropdown(menuItems: NodeListOf<Element>, e: MouseEvent) {
  for (const x of menuItems) {
    const item = x as HTMLElement
