...
class ImageInputComponent {
  ...
  private _change = (e: Event) => {
    ...
    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      ...
      reader.onload = (innere: ProgressEvent<FileReader>) => {
        if (this.wrapperElement && innere.target) {
          this.wrapperElement.style.setProperty('background-image', `url('${innere.target.result}')`)
        }
      }
