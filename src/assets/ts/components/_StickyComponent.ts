...
class StickyComponent {
  ...
  private scroll = () => {
    ...
    // Reverse scroll mode
    if (reverse === true) {
      this.handleReverseScrollMode( st, offsetNum)
    }

    // Classic scroll mode
    this.handleClassicScrollMode(st, offsetNum)
  }
  private handleReverseScrollMode = ( st: number, offsetNum: number) => {
  ...
  private handleClassicScrollMode = (st: number, offsetNum: number) => {
    if (st > offsetNum) {
      if (document.body.hasAttribute(this.attributeName) === false) {
        this.enable()
        document.body.setAttribute(this.attributeName, 'on')
  ...
  private enable = (update: boolean = false) => {
    ...
    if (update && !this.getOption('animation')) return;
    ...
    this.setElementWidthByWidthValue(width)
  ...
  private setElementWidthByWidthValue(width: string | boolean | JSON | undefined) {
