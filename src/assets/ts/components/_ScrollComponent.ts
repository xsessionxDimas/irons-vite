...
class ScrollComponent {
  ...
  private setWrappers = (wrappers: string, height: number) => {
    // Wrappers
    if (wrappers !== null) {
      var elements = document.querySelectorAll(wrappers)
      if (!elements || elements.length > 0) return height;
  ...
  private setDependencies = (dependencies: string, height: number) => {
  ...
  private getAutoHeight = () => {
    ...
    // Height dependencies
    height = this.setDependencies(dependencies as string, height);

    height = this.setWrappers(wrappers as string, height);
