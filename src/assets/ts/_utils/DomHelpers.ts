...
function slide(el: HTMLElement, dir: string, speed: number, callback: any) {
  ...
  handleAnimateElementByDir(dir, el, calcPaddingTop, calcPaddingBottom, speed, calcHeight, callback)
}

function handleAnimateElementByDir(dir: string, el: HTMLElement, calcPaddingTop: number, calcPaddingBottom: number, speed: number, calcHeight: number | undefined, callback: any) {
  if (dir === 'up') {
    // up
    el.style.cssText = 'display: block; overflow: hidden;'

    handleCalcPaddingDirUp(calcPaddingTop, calcPaddingBottom, speed, el)
    ...
    handleCalcPaddingDirDown(calcPaddingTop, calcPaddingBottom, speed, el)
...
function handleCalcPaddingDirUp(calcPaddingTop: number, calcPaddingBottom: number, speed: number, el: HTMLElement) {
  if (calcPaddingTop) {
...
function handleCalcPaddingDirDown(calcPaddingTop: number, calcPaddingBottom: number, speed: number, el: HTMLElement) {
  if (calcPaddingTop) {
    ElementAnimateUtil.animate(
      0,
      calcPaddingTop,
...
function colorLighten(color: string, amount: number) {
  const addLight = (_color: string, _amount: number) => {
    ...
    return cNum.toString(16).length > 1 ? cNum.toString(16) : `0${cNum.toString(16)}`;
  }
