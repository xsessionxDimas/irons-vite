<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataAssignmentLocationCrack.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/crack-assignment/assignment-location-crack/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (e) => {
    ...
    return {
      model: item.model,
      psType: item.psType,
      taskNumberDetailParent: item.taskNumberDetailParent,
      locationIdParent: item.locationIdParent,
      taskNumberDetailChild: item.taskNumberDetailChild,
      locationIdChild: item.locationIdChild,
...
const handleDownload = () => {
  ...
  const columns = [
    ...
    "PS TYPE",
    "LOCATION ID PARENT",
    "TASK NUMBER DETAIL PARENT",
    "LOCATION ID CHILD",
    "TASK NUMBER DETAIL CHILD",
    "START DATE",
    "END DATE",
    "REMARK",
    "STATUS",
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      model: val.model,
      psType: val.psType,
      taskNumberDetailParent: val.taskNumberDetailParent,
      locationIdParent: val.locationIdParent,
      taskNumberDetailChild: val.taskNumberDetailChild,
      locationIdChild: val.locationIdChild,
  ...
  handleDataToExcel("AssignmentLocationCrack", columns, data);
}
