...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Assignment Location Crack", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
    {
      pageName: "Parameter",
      pageRoute: "assignmentlocationcrack",
    },
    {
      pageName: "Assignment Location Crack",
      pageRoute: "assignmentlocationcrack",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getLocationCrackLookup();
  await formStore.getModelCrackLookup();
  await formStore.getPsTypeLookup();
  await formStore.getTaskCrackLookup();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `Assignment Location Crack.xlsx`);
}
