<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataTaskCrack.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/crack-assignment/task-crack/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      psType: item.psType,
      taskNo: item.taskNo,
      taskCrackCode: item.taskCrackCode,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      psType: val.psType,
      taskNo: val.taskNo,
      taskCrackCode: val.taskCrackCode,
  ...
  handleDataToExcel("Master Data Task Crack", columns, data);
}
