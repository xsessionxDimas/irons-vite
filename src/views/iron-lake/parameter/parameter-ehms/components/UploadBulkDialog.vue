<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/Parameter.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/parameter/parameter-ehms/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      CbmArea: item.cbmArea,
      CbmParameterId: item.cbmParameterId,
      Model: item.model,
      PsType: item.psType,
      ...
      TypeParameterId: item.typeParameterId,
      IsActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      CbmParameterId: val.cbmParameterId,
      ComponentId: val.componentId,
      TypeParameterId: val.typeParameterId,
  ...
  handleDataToExcel("Parameter", columns, data);
}
