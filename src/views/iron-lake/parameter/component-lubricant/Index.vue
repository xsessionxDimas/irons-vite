...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Component Lubricant", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
    {
      pageName: "Parameter",
      pageRoute: "componentlubricant",
    },
    {
      pageName: "Component Lubricant",
      pageRoute: "componentlubricant",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getFormOptions("component");
  await formStore.getFormOptions("uom");
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `Component Lubricant.xlsx`);
}
