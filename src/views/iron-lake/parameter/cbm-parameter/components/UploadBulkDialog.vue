<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataCbmParameter.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/parameter/cbm-parameter/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/parameter/cbm-parameter/BulkItem";
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      CbmParameterName: val.cbmParameter,
  ...
  handleDataToExcel("CBM Parameter", columns, data);
}
