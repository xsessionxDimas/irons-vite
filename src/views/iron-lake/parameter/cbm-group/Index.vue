...
import {
  useCbmGroupFormStore
} from "@/store/pinia/iron-lake/parameter/cbm-group/useCbmGroupFormStore";
import { saveAs } from "file-saver";

const store = useStore();
const listStore = useCbmGroupListStore();
const formStore = useCbmGroupFormStore();
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("CBM Group", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `CBM Group.xlsx`);
}
