<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataStatusConverter.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useStatusConverterBulkStore
} from "@/store/pinia/iron-lake/parameter/status-converter/useStatusConverterBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/parameter/status-converter/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/parameter/status-converter/BulkItem";
...
const bulkStore = useStatusConverterBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      statusConverterId: "0",
      statusConverter: item.statusConverter,
      statusConverterDescription: item.statusConverterDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      statusConverter: val.statusConverter,
      statusConverterDescription: val.statusConverterDescription,
  ...
  handleDataToExcel("Master Data Status Converter", columns, data);
}
