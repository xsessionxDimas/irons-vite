<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/OilToleranceSetting.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useOilToleranceSettingBulkStore
} from "@/store/pinia/iron-lake/parameter/oil-tolerance-setting/useOilToleranceSettingBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/parameter/oil-tolerance-setting/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/parameter/oil-tolerance-setting/BulkItem";
...
const bulkStore = useOilToleranceSettingBulkStore();
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      valueMin: String(val.valueMin),
      valueMax: String(val.valueMax),
      uom: val.uom,
  ...
  handleDataToExcel("Oil Tolerance Setting", columns, data);
}
