<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataUserDirect.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/administration/organization-unit/user-direct/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (e) => {
    ...
    return {
      md_employee_direct_id: 0,
      EmployeeId: item.EmployeeId,
      Name: item.Name,
      EmployeeIdDirect: item.EmployeeIdDirect,
      EmployeeDirectName: item.EmployeeDirectName,
      IsActive: true,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      name: val.Name,
      employeeIdDirect: val.EmployeeIdDirect,
      employeeIdDirectName: val.EmployeeDirectName,
  ...
  handleDataToExcel("UserDirect", columns, data);
}
