...
<template>
  <el-dialog
    ...
    <el-form
      ...
      <div class="col-12 fv-row fv-plugins-icon-container">
        <SelectInput :margin="false" :options="companies" :value="formData.company" name="Company" label="Company" placeholder="Select company"
          required @handleChange="handleCompanyChange" />
      </div>
      <div class="col-12 fv-row fv-plugins-icon-container" v-if="formData.company?.toLocaleLowerCase() === 'external'">
      ...
      <div class="col-12 fv-row fv-plugins-icon-container">
          <el-form-item class="m-0">
            <template #label>
              <span>{{ formData.company === 'BUMA AU' ? 'BUMA AU Email' : 'Email'}}</span>
            </template>
            <el-input
              v-model="formData.email"
              :placeholder="`Write ${formData.company === 'BUMA AU' ? 'BUMA AU ' : 'Employee '}Email`"
      ...
      <div class="col-12 fv-row fv-plugins-icon-container">
          <div class="align-items-center fv-row">
            <label :class="`form-label fs-6 fw-bolder`">Status <span class="text-danger">*</span></label>
            <div class="d-flex align-items-center justify-content-between form-control form-control-lg">
              <el-switch v-model="formData.isActive" name="Status" @handleChange="handleStatusChange" />
              <span>
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button class="btn btn-outline btn-outline-btech-secondary btn-active-light-btech-secondary" @click="handleCloseModal()" :disabled="formStore.loading">Cancel</el-button>
        <el-button class="btn btn-btech-secondary" @click="handleSubmitData" :disabled="formStore.loading">Submit</el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
const isActive = computed(() => {
  const value = formData.value.isActive
...
const handleSubmitData = async () => {
  ...
  if (previewPhoto.value) {
    formStore.uploadImage().then(() => {
      if (!formStore.isError) {
        formStore.insertData(formStore.stateUploadedImageData.url).then(() => {
          if (!formStore.isError) {
            swalAlertSuccess("Record has been submitted successfully", "Ok")
              .then(() => {
    ...
    formStore.insertData('').then(() => {
      if (!formStore.isError) {
        swalAlertSuccessTitle("Record has been submitted successfully", "", "Ok")
          .then(() => {
            handleCloseModal(true);
          });
      } else {
