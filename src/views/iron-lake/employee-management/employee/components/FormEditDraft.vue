...
const formData: ComputedRef<UpsertDraft> = computed(() => {
  ...
  if (previewPhoto.value) {
    formStore.uploadImage().then(() => {
      if (!formStore.isError) {
        formStore.updateDraft(formStore.stateUploadedImageData.url).then(() => {
          if (!formStore.isError) {
            swalAlertSuccessTitle("Upload Success", "", "Ok")
              .then(() => {
        ...
        swalAlertErrorTitle("Upload Failed", "", "Ok");
      }
    });
  } else {
