<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/Outstanding Service.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/task/daily-schedule-v1/BulkItem";
...
const handleDownload = () => {
  ...
  handleDataToExcel("Outstanding Service", columns, data);
}
  ...
  swalAlertConfirmation("Are you sure you want to submit ?").then(async (res) => {
    if (res.isConfirmed) {
      ...
      if (bulkData.length == 0) {
        swalAlertErrorBulk("There is no valid data to submit");
        return
      }
      ...
      swalAlertSuccess("Outstanding service schedule has been successfully uploaded !", "Ok")
        .then(() => {
          handleClose(true);
        });
    }
