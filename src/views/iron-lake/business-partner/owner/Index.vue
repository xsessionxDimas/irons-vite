...
import {
  useOwnerFormStore
} from "../../../../store/pinia/iron-lake/business-partner/owner/useOwnerFormStore";
import { saveAs } from "file-saver";

const store = useMenuStore();
const listStore = useOwnerListStore();
const formStore = useOwnerFormStore();
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Owner", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
  ...
  await formStore.getCompanyLookup();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `Owner.xlsx`);
}
