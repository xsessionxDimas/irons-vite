<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="/documents/MasterDataMaintenancePlant.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useMaintenancePlantBulkStore
} from "@/store/pinia/iron-lake/business-partner/maintenance-plant/useMaintenancePlantBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/business-partner/maintenance-plant/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/business-partner/maintenance-plant/BulkItem";
...
const bulkStore = useMaintenancePlantBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      maintenancePlantId: item.maintenancePlantId,
      maintenancePlantDescription: item.maintenancePlantDescription,
      maintenancePlantCode: item.maintenancePlantCode,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      maintenancePlant: val.maintenancePlantId,
      maintenancePlantCode: val.maintenancePlantCode,
      maintenancePlantDescription: val.maintenancePlantDescription,
  ...
  handleDataToExcel("Maintenance Plant", columns, data);
}
