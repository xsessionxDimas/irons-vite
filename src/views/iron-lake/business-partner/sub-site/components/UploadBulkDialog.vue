<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataSubSite.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useSubSiteBulkStore
} from "@/store/pinia/iron-lake/business-partner/sub-site/useSubSiteBulkStore";
import {
...
const bulkStore = useSubSiteBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      subSiteDescription: item.subSiteDescription,
      isActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      subSite: val.subSite,
      subSiteDescription: val.subSiteDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Sub Site", columns, data);
}
