<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataBrand.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useBrandBulkStore
} from "@/store/pinia/iron-lake/business-partner/brand/useBrandBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/business-partner/brand/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/business-partner/brand/BulkItem";
...
const bulkStore = useBrandBulkStore();
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      brand: val.brand,
      brandDescription: val.brandDescription,
  ...
  handleDataToExcel("Master Data Brand", columns, data);
}
