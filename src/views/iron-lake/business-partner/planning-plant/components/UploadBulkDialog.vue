<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="/documents/MasterDataPlanningPlant.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/business-partner/planning-plant/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/business-partner/planning-plant/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      planningPlantId: item.planningPlantId,
      planningPlantCode: item.planningPlantCode,
      planningPlantDescription: item.planningPlantDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      planningPlantId: val.planningPlantId,
      planningPlantCode: val.planningPlantCode,
      planningPlantDescription: val.planningPlantDescription,
  ...
  handleDataToExcel("Planning Plant", columns, data);
}
