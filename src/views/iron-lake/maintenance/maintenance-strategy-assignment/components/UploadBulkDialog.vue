<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataMaintenanceStartegyAssignment.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useMaintenanceStrategyAssignmentBulkStore
} from "@/store/pinia/iron-lake/maintenance/maintenance-strategy-assignment/useMaintenanceStrategyAssignmentBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/maintenance/maintenance-strategy-assignment/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/maintenance/maintenance-strategy-assignment/BulkItem";
...
const bulkStore = useMaintenanceStrategyAssignmentBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      maintenanceStrategyParId: item.maintenanceStrategyParId,
      maintenanceStrategyChdId: item.maintenanceStrategyChdId,
...
const handleDownload = () => {
  ...
  const columns = [
    "ID Maintenance Strategy Parent",
    "ID Maintenance Strategy Child",
    "Start Date",
    "End Date",
    "Remark",
    "Status"
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      id: val.maintenanceStrategyParId,
      strategyCategory: val.maintenanceStrategyChdId,
  ...
  handleDataToExcel("Maintenance Strategy Assignment", columns, data);
}
