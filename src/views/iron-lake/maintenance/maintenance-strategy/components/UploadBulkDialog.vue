<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
        Download template
        <a href="documents/MaintenanceStrategy.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/maintenance/maintenance-strategy/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      maintenanceStrategyId: item.maintenanceStrategyId,
      strategyCategory: item.strategyCategory,
      strategyPackage: item.strategyPackage,
      budgetLife: item.budgetLife,
      uom: item.uom,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      id: val.maintenanceStrategyId,
      strategyCategory: val.strategyCategory,
      strategyPackage: val.strategyPackage,
      budgetLife: val.budgetLife,
      uom: val.uom,
  ...
  handleDataToExcel("Master Maintenance Strategy", columns, data);
}
