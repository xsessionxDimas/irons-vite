<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataWBSElement.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useWbsElementBulkStore
} from "@/store/pinia/iron-lake/equipment/wbs-element/useWbsElementBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/wbs-element/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/wbs-element/BulkItem";
...
const bulkStore = useWbsElementBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      wbsElementDescription: item.wbsElementDescription,
      isActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      wbsElementDescription: val.wbsElementDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("WBS Element", columns, data);
}
