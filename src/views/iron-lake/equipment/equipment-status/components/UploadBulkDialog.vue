<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataEquipmentStatus.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/equipment-status/BulkItem";
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      systemStatus: val.systemStatus,
      status: val.status,
      subStatus: val.subStatus,
      startDate: val.startDate,
      endDate: val.endDate,
      remark: val.validationReason,
      valid: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("EquipmentStatus", columns, data);
}
