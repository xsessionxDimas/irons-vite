<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataEquipmentGroup.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useEquipmentGroupBulkStore
} from "@/store/pinia/iron-lake/equipment/equipment-group/useEquipmentGroupBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/equipment-group/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/equipment-group/BulkItem";
...
const bulkStore = useEquipmentGroupBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      equipmentGroupDescription: item.equipmentGroupDescription,
      isActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      equipmentGroupDescription: val.equipmentGroupDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("EquipmentGroup", columns, data);
}
