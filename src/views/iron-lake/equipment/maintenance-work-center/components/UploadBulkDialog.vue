<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataMaintenanceWorkCenter.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useMaintenanceWorkCenterBulkStore
} from "@/store/pinia/iron-lake/equipment/maintenance-work-center/useMaintenanceWorkCenterBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/maintenance-work-center/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/maintenance-work-center/BulkItem";
...
const bulkStore = useMaintenanceWorkCenterBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      maintenanceWorkCenterId: 0,
      maintenanceWorkCenter: item.maintenanceWorkCenter,
      maintenanceWorkCenterDescription: item.maintenanceWorkCenterDescription,
      isActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      maintenanceWorkCenterDescription: val.maintenanceWorkCenterDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Maintenance Work Center", columns, data);
}
