<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataCostCenter.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useCostCenterBulkStore
} from "@/store/pinia/iron-lake/equipment/cost-center/useCostCenterBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/cost-center/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/cost-center/BulkItem";
...
const bulkStore = useCostCenterBulkStore();
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      costCenterDescription: val.costCenterDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Cost Center", columns, data);
}
