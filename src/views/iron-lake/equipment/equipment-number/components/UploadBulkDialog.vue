<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataEquipmentNumber.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/equipment-number/BulkItem";
import {
  formatDateOnlyAU
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      level: item.level,
      equipmentNumberDescription: item.equipmentNumberDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      equipmentNumberDescription: val.equipmentNumberDescription,
  ...
  handleDataToExcel("EquipmentNumber", columns, data);
}
