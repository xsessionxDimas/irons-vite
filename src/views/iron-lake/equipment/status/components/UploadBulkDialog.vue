<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataStatus.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/status/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/status/BulkItem";
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      isValid: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Status", columns, data);
}
