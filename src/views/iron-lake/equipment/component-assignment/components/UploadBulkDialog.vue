<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataComponentAssignment.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useComponentAssignmentBulkStore
} from "@/store/pinia/iron-lake/equipment/component-assignment/useComponentAssignmentBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/component-assignment/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/component-assignment/BulkItem";
...
const bulkStore = useComponentAssignmentBulkStore();
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      startDate: formatDateOnlyAU(val.startDate),
      endDate: formatDateOnlyAU(val.endDate),
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Component Assignment", columns, data);
}
