...
import {
  useCharacteristicTypeValueBulkStore
} from "@/store/pinia/iron-lake/equipment/characteristic-type-value/useCharacteristicTypeValueBulkStore";
import {
  useCharacteristicTypeValueFormStore
} from "@/store/pinia/iron-lake/equipment/characteristic-type-value/useCharacteristicTypeValueFormStore";
import { saveAs } from "file-saver";

const store = useStore();
const listStore = useCharacteristicTypeValueListStore();
const formStore = useCharacteristicTypeValueFormStore();
const bulkStore = useCharacteristicTypeValueBulkStore();
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Characteristic Type Value", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
    {
      pageName: "Equipment",
      pageRoute: "characteristictypevalue",
    },
    {
      pageName: "Characteristic Type Value",
      pageRoute: "characteristictypevalue",
    },
  ]);
  listStore.setPage(1);
  listStore.getLookupType();
  listStore.getLookupValue();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `Characteristic Type Value.xlsx`);
}
