<template>
    ...
    <template v-else>
      ...
      :empty-text="'No Data'">
          ...
          <el-table-column prop="equipmentTypeDescription" label="Equipment Type Desc." width="200" sortable :sort-method="(a, b) => customSort(a, b, 'equipmentTypeDescription')" />
          ...
          <el-table-column prop="changedOn" label="Changed on" align="center" sortable>
              <template #default="scope">
                  <span>{{ formatDateAU(scope.row.changedOn) }}</span>
              </template>
          </el-table-column>
          <el-table-column prop="changedBy" label="Changed by" sortable :sort-method="(a, b) => customSort(a, b, 'changedBy')" />
