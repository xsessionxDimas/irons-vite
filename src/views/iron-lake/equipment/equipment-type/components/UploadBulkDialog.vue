<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataEquipmentType.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useEquipmentTypeBulkStore
} from "@/store/pinia/iron-lake/equipment/equipment-type/useEquipmentTypeBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/equipment-type/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/equipment-type/BulkItem";
...
const bulkStore = useEquipmentTypeBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      equipmentTypeDescription: item.equipmentTypeDescription,
      isActive: true
    } as BulkItem
  });
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      equipmentTypeDescription: val.equipmentTypeDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Equipment Type", columns, data);
}
