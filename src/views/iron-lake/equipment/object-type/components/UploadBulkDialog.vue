<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataObjectType.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/object-type/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/object-type/BulkItem";
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      objectTypeDescription: val.objectTypeDescription,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("ObjectType", columns, data);
}
