<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterData-ComponentType.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useComponentTypeBulkStore
} from "@/store/pinia/iron-lake/equipment/component-type/useComponentTypeBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/component-type/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/component-type/BulkItem";
...
const bulkStore = useComponentTypeBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (e) => {
    ...
    return {
      ...
      componentTypeDescription: item.componentTypeDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      componentTypeDescription: val.componentTypeDescription,
  ...
  handleDataToExcel("ComponentType", columns, data);
}
