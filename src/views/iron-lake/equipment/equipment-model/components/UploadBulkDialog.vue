<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataEquipmentModel.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useEquipmentModelBulkStore
} from "@/store/pinia/iron-lake/equipment/equipment-model/useEquipmentModelBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-lake/equipment/equipment-model/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-lake/equipment/equipment-model/BulkItem";
...
const bulkStore = useEquipmentModelBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (e) => {
    ...
    return {
      ...
      equipmentModelDescription: item.equipmentModelDescription,
      brand: item.brand,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      eqipmentModel: val.equipmentModel,
      eqipmentModelDescription: val.equipmentModelDescription,
      brand: val.brand,
  ...
  handleDataToExcel("EquipmentModel", columns, data);
}
