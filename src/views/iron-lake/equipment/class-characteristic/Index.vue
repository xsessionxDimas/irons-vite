...
import {
  useClassCharacteristicBulkStore
} from "@/store/pinia/iron-lake/equipment/class-characteristic/useClassCharacteristicBulkStore";
import {
  useClassCharacteristicFormStore
} from "@/store/pinia/iron-lake/equipment/class-characteristic/useClassCharacteristicFormStore";
import { saveAs } from "file-saver";

const store = useStore();
const listStore = useClassCharacteristicListStore();
const formStore = useClassCharacteristicFormStore();
const bulkStore = useClassCharacteristicBulkStore();
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Class Characteristic", [
    {
      pageName: "Iron Lake",
      pageRoute: "ironlake",
    },
    {
      pageName: "Equipment",
      pageRoute: "classcharacteristic",
    },
    {
      pageName: "Class Characteristic",
      pageRoute: "classcharacteristic",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getClassLookup();
  await formStore.getCharacteristicTypeLookup();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `ClassCharacteristic.xlsx`);
}
