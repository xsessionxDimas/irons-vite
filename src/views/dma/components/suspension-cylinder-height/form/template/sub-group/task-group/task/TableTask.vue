...
<div class="row w-100 mx-0 mb-5">
  ...
  <div class="task-table" :class="!isUndefined(task.style) && task.style.category == 'taskTable' ? 'col-11 px-0' : 'col-12 pe-0'">
    ...
    <div v-for="(row, index) in task.items" :key="row.key" class="row w-100 task-table__body d-flex align-items-start" :class="taskRowBorder(index)">
      <TableRow :item="row" :task="task" :item-loading="itemLoading"/>
    </div>
  </div>
</div>
</template>
