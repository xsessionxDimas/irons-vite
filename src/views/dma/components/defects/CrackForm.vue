  ...
  >
    ...
    <div>
                  ...
                  <div class="row p-0 m-0 h-50 w-100">
                    <div class="p-0 col-12 d-flex">
                      <input class="form-control" :value="formatNumberInput(item.qty)" @input="onPartQtyChange($event, index)" :disabled="isDisabled"
                        pattern="[0-9]*" inputmode="numeric" @keypress="onlyNumber"
                      />
                    </div>
                  </div>
          ...
          <template v-for="(item, index) in data.Labours" :key="item">
            <div class="row p-3 m-1 priority-container px-0">
              ...
              <div class="col-2  d-flex justify-content-start">
                <div class="d-flex flex-column">
                  <input class="form-control" :value="formatNumberInput(item.qty)" pattern="[0-9]*" inputmode="numeric" @keypress="onlyIntegerInput"
              ...
              <div class="col-2 d-flex justify-content-start">
                <div class="d-flex flex-column">
                  <input class="form-control" :value="formatNumberInput(item.hireEach)" pattern="[0-9]*" inputmode="numeric" @keypress="onlyNumber"
                  @input="onLabourHireEachChange($event, index)" :disabled="isDisabled" />
      ...
      <div class="my-5 w-100 d-flex justify-content-around" v-if="computedDefectStatus">
        <button v-if="!showCancelButtonOnModify" class="btn btn-outline w-100" @click.prevent="() => toggleShowDeclineRease(true)"
        ...
        <template v-if="!showCancelButtonOnModify">
          <button class="ms-3 btn btn-warning w-100" @click.prevent="handleModifyAndShowCancelButton"
...
} from '../../e-form-offline/types/ChangeDefectVersionParam'
import {
  safelyParseJSON
...
import {
  CompleteEmitParam
} from '../../e-form-offline/types/CompleteEmitParam'
...
const computedDefectStatus = computed(() => {
  ...
  const found = formStore.ApprovalData.CrackHeaders.find((x) => {
...
const completeMessage = computed(() => {
  ...
  if (props.status == "Decline") {
    message = "Crack Identified Successfully Declined"
    return message
  }
  if (!updateDataOnly.value) {
    if (isDeclineClicked.value) {
      message = "Crack Declined"
      return message;
    }
  ...
  if (!updateDataOnly.value) {
    if (defectSheetStore.SelectedFitter.name) {
      if (showCancelButtonOnModify.value) {
        const result = await formStore.updateCrackFitter(JSON.stringify(defectSheetStore.SelectedFitter), "")
        if (isError.value) {
      ...
      const result = await formStore.updateCrackDetail()
      if (isError.value) {
        errorMessage.value = result
        loader.close()
        errorMessageBoxVisible.value = true
      ...
      await formStore.updatePlannerCrackAcknowledge()
    } else {
      const result = await formStore.updateCrackDetail()
      if (isError.value) {
        errorMessage.value = result
      ...
      await formStore.updateCrackAcknowledge()
    }
  }
  if (isError.value) {
    loader.close()
