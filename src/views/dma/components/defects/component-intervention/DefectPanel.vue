<template>
                    ...
                    <div class="row flex-nowrap px-2 my-2 mx-1 w-100">
                      ...
                      <div class="col-1 column-sm-20-percent column-md-24-percent column-lg-10-percent column-xl-10-percent align-items-center d-flex justify-content-center">
                        <div v-if="!isDecline(item)" class="label d-flex align-items-center acknowledge-btn py-2 px-1 rounded text-center justify-content-center cursor-pointer"
                          :class="formClass(item.id, headers, plannerApprove ? true : false)"
                      ...
                      <div class="col-1 column-sm-10-percent column-md-6-percent column-lg-6-percent column-xl-6-percent justify-content-center align-items-center d-flex">
                        ...
                        <div class="mx-3 little-box rounded-circle d-flex p-3 justify-content-center align-items-center mx-auto" style="margin-top: 6px;"
                          :class="[repairedClass(item), allowedClass(item.priority)]" @click="updateRepaired(item.id, item.priority)">
    ...
    :decline-by="store.DeclineBy"
    :decline-date="store.DeclineDate"
...
const titlePanel = ref(props.isAdditionalTask ? 'Defect Identified (Additional Task)' : 'Defect Identified')
...
const headers = computed(() => {
  const defectHeader = store.ApprovalData.DefectHeaders
...
const handleFormatTimeStamp = (defectHeader) => {
  ...
  console.log("data senua", store.ApprovalData)
...
const updateRepaired = async (headerId: string, priority: string | undefined) => {
  ...
  const header = store.ApprovalData.DefectHeaders.find((h) => {
    return h.id === headerId;
  })
  if (header) {
    const value = header.repairedStatus == "Repaired" ? "Not-Repaired" : "Repaired";
    await store.updateRepairedStatus(headerId, value);
  }
}
