...
<div class="row w-100" :style="[generalStyle, taskCustomStyle]">
  <template v-for="item in props.task.items" :key="item.key">
    ...
    <template v-if="item.itemType === 'textAreaInput'">
      <TextAreaInputPrint v-if="isDownloadPDF" :item="item" />
      <TextAreaInput v-else :item="item" :task="task"/>
    </template>
    <template v-if="item.itemType === 'label'">
      <LabelPrint v-if="isDownloadPDF" :item="item" :task="task"/>
      <Label v-else :item="item" :task="task"/>
    </template>
  </template>
</div>
...
import TextAreaInputPrint from './item/TextAreaInputPrint.vue';
import LabelPrint from './item/LabelPrint.vue';
...
const historicalEformDmaStore = useHistoricalEformDmaStore()

const isDownloadPDF = computed(() => {
  return historicalEformDmaStore.IsDownloadPDF
})
