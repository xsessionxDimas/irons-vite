<template>
    ...
    <el-collapse class="task-group general-accordion defect-panel-wrapper px-5 pb-1" v-model="defectIdentifiedCollapse">
      <el-collapse-item :title="`Generic Defect Identified - ${title}`" name="Generic Defect Identified">
              ...
              <template v-else v-for="header in headers" :key="header">
                <div class="item px-2 my-2 mx-1" :class="backgroundPanelColor(header)">
                  <div class="row flex-nowrap px-2 my-2 mx-1 w-100">
                    ...
                    <div class="col-1 column-sm-20-percent column-md-24-percent column-lg-10-percent column-xl-10-percent align-items-center d-flex justify-content-center">
                      ...
                      <template v-else>
                        <div class="label d-flex align-items-center acknowledge-btn py-2 px-1 rounded text-center justify-content-center cursor-pointer"
                          ...
                          @click="acknowledge(header.defectHeaderId)">
                          {{ header.isRevised == "true" ? "Revision Approval" : "Detailed Information" }}
                        </div>
                      </template>
                    </div>
                      ...
                      <div class="row">
                        <div class="col-8 col-md-8 d-flex flex-column pe-0">
                          <input type="text" class="form-control mou-input"
                            ...
                            @keypress="onlyNumber"
                            autocomplete="false"
                            :value="woValue(header.defectHeaderId)"
                            @input.prevent="onChangeWoValue"
                            @change="(e) => handleChangeWoValue(e, header.defectHeaderId)"
                            :disabled="defectMoFillable(header.defectHeaderId, headers, fitterId, header)" />
                        ...
                        <div class="col-4 col-md-3 ps-0 pe-1">
                          <button :disabled="defectMoFillable(header.defectHeaderId, headers, fitterId, header)"
                  ...
                  <div v-if="viewIsDownload" class="col-1 column-sm-10-percent column-md-6-percent column-lg-6-percent column-xl-6-percent justify-content-center align-items-center d-flex"
                    ...
                    <div class="mx-3 little-box rounded-circle d-flex p-3 justify-content-center align-items-center mx-auto" style="margin-top: 6px;cursor: initial">
                      <img v-if="viewIconDownload(header.defectWorkorder && header.defectWorkorder != '', plannerApprove ? header.plannerStatus : header.status)" @click="handleConfirmationPrintPDF(header.defectHeaderId)" class="ms-4 me-2 download-icon" src="/media/svg/dma/document_download.svg" :class="!isDownloaded(header.downloadHistory) ? 'icon-red':'icon-green'" style="height: 18px" alt="">
                    </div>
                  </div>
                  </div>
  ...
  <DefectConfirmationDialog :show="confirmSubmitVisibility"
    ...
    @close="onCancel"
   />
  <DefectsYesForm :visibility="defectYesVisibility" :asset-number="assetNumber"
...
import DefectsYesForm from "@/views/dma/e-form/defects/DefectsYesForm.vue";
...
const showDefectForm = () => {
  ...
  if (checkBoxList.value.length < 1) {
    ...
    const need30MntsLabel = "Does the defect identified require > 30 minutes in labour to repair?"
    if (checkBoxList.value.includes(need30MntsLabel)) {
      defectStore.toggleIsNeed30Minutes(true)
    }
    onConfirmSubmit()
  }
  checkBoxList.value = []
...
const defectNoVisibility = computed(() => {
  return defectStore.DefectNoVisible;
});
...
const headers = computed(() => {
  ...
  return store.ApprovalData.DefectGenericHeaders.filter((header) => {
...
const defectMoFillable = (headerId: string, headers: Header[], fitterId = "", defect: any): boolean => {
  let disabled = true
...
const detailInfoClass = (headerId: string, headers: Header[]) => {
  ...
  if (props.supervisor) {
    if (header) {
      ...
      cssClass = header.status === 'Acknowledge' ? "light-green color-dark-green" : "light-yellow color-dark-yellow"
