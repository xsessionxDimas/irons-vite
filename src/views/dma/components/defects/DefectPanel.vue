                    ...
                    <el-collapse-item :name="headersDisplay[index][0].taskNo">
                      <template #title>
                        <div class="row mt-2 flex-nowrap px-2 my-2 mx-1 w-100">
                          <span v-if="headersDisplay[index][0].taskNo" class="col-1 column-sm-12-percent column-md-7-percent column-xl-6-percent align-items-center d-flex text-break justify-content-center">{{ headersDisplay[index][0].taskNo.split(';')[0] }}</span>
                      ...
                      <div v-for="defect in headersDisplay[index]" :key="defect.id">
                        <div class="item px-2 my-2 mx-1" :class="backgroundPanelColor(defect)">
                          <div class="row flex-nowrap px-2 my-2 mx-1 w-100">
                            ...
                            <div class="col-1 column-sm-20-percent column-md-24-percent column-lg-10-percent column-xl-10-percent align-items-center d-flex justify-content-center">
                              ...
                              <template v-else>
                                <div class="label d-flex align-items-center acknowledge-btn py-2 px-1 rounded text-center justify-content-center cursor-pointer"
                                  ...
                                  @click="acknowledge(defect.id)">
                                  {{ defect.isRevised == "true" ? "Revision Approval" : "Detailed Information" }}
                                </div>
                              </template>
                            </div>
                              ...
                              <div class="row">
                                <div class="col-8 col-md-8 d-flex flex-column pe-0">
                                  <input type="text" class="form-control mou-input"
                                    ...
                                    :disabled="defectMoFillable(defect.id, headers, fitterId, defect)" />
                                ...
                                <div class="col-4 col-md-3 ps-0 pe-1">
                                  <button :disabled="defectMoFillable(defect.id, headers, fitterId, defect)"
                          ...
                          <div v-if="viewIsDownload" class="col-1 column-sm-10-percent column-md-6-percent column-lg-6-percent column-xl-6-percent justify-content-center align-items-center d-flex"
                            ...
                            <div class="mx-3 little-box rounded-circle d-flex p-3 justify-content-center align-items-center mx-auto" style="margin-top: 6px;cursor: initial">
                              <img v-if="viewIconDownload(plannerApprove ? defect.plannerStatus : defect.status)" @click="handleConfirmationPrintPDF(defect.id, headersDisplay[index][0].taskDesc, defect.defectType)" class="ms-4 me-2 download-icon" src="/media/svg/dma/document_download.svg" :class="!isDownloaded(defect.downloadHistory) ? 'icon-red':'icon-green'" style="height: 18px" alt="">
                            </div>
                          </div>
                          </div>
                  ...
                  <div v-else>
                    <div class="item header-collapse px-2 my-2 mx-1">
                      <div class="row flex-nowrap px-2 my-2 mx-1 w-100">
                        ...
                        >
                        <div class="mx-3 little-box rounded-circle d-flex p-3 justify-content-center align-items-center mx-auto" style="margin-top: 6px;"
                          :class="[completeClass(headersDisplay[index][0])]">
                          <img src="/media/svg/dma/icons/Repair.png" alt="" />
                        </div>
                      </div>
                      <!-- download -->
                      <div v-if="viewIsDownload" class="col-1 column-sm-10-percent column-md-6-percent column-lg-6-percent column-xl-6-percent justify-content-center align-items-center d-flex">                      </div>
                      </div>
                      <div class="row flex-nowrap w-100 flex-row-reverse create-by" style="font-size: 13px !important">
...
const handleDownloadPDF = () => {
  ...
  if (data && header) {
    const isDataPrepared = prepareDownloadableData({
      ...
      taskDesc: selectedDefectDownload.value.taskDesc
    })
    if (isDataPrepared) {
