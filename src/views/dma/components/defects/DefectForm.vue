<template>
  <el-dialog v-model="dialogVisible"
    ...
    <div>
      ...
      <div class="d-flex flex-row justify-content-between">
        <large-camera :id="'defect'" :disabled="isDisabled" :allow-delete="true" :is-mandatory="false"
          :is-monitoring="isDisabled" />
      </div>
      <div class="mt-1 py-2">
        ...
        <div class="row">
          ...
          <div class="col pe-0 d-flex">
            <div class="d-flex flex-column flex-fill">
              <div class="form-floating">
                <input type="text" class="form-control" @input.prevent="onPlannedHoursChange" @keypress="onlyNumber"
                  :value="formatNumberInput(data.PlannedDuration.value)" :disabled="isDisabled" pattern="[0-9]*"
                  inputmode="numeric" />
      ...
      <el-collapse class="task-group general-accordion py-1 px-5 parts-collapse" v-model="collapsibleParts">
        <el-collapse-item title="Parts" name="Parts" id="partsSection">
              ...
              <div class="col-1 px-1 d-flex flex-column justify-content-start">
                <div class="row p-0 m-0 h-50 w-100">
                  <div class="p-0 col-12 d-flex">
                    <input class="form-control" :value="formatNumberInput(item.qty)"
                      @input="onPartQtyChange($event, index)" :disabled="isDisabled" pattern="[0-9]*"
                      inputmode="numeric" @keypress="onlyNumber" />
                  </div>
                </div>
              ...
              <div class="col px-1 d-flex flex-column justify-content-start position-relative">
                ...
                <div class="position-absolute" style="top:10px; right: 0px;" :class="isDisabled ? 'hidden' : ''">
                  <em class="fa fa-times" style="color:gray; cursor: pointer;"
                    @click.prevent="onRemoveParts(index)"></em>
                </div>
              </div>
            </div>
          </template>
          <div class="d-flex justify-content-between align-items-center flex-row-reverse">
            <div class="fw-bolder">
              Check Partbook
              <a :href="defectformStore.statePartReference.linkAddress" target="_blank" class="fw-bolder" style="color: #18AF4A;">
                <img src="/media/icons/bumaau/open_in_new.svg"/>
                {{ defectformStore.statePartReference.brand }}
              </a>
            </div>
          ...
          <template v-for="(item, index) in data.Labours" :key="item">
            <div class="row p-3 m-1 priority-container px-0">
              ...
              <div class="col-2 d-flex justify-content-start">
                <div class="d-flex flex-column">
                  <input class="form-control" :value="formatNumberInput(item.qty)" pattern="[0-9]*" inputmode="numeric"
                    @keypress="onlyIntegerInput" @input="onLabourQtyChange($event, index)" :disabled="isDisabled" />
              ...
              <div class="col-2 d-flex justify-content-start">
                <div class="d-flex flex-column">
                  <input class="form-control" :value="formatNumberInput(item.hireEach)" pattern="[0-9]*"
                    inputmode="numeric" @keypress="onlyNumber" @input="onLabourHireEachChange($event, index)"
                    :disabled="isDisabled" />
            ...
            <div class="d-flex flex-row">
              <div class="d-flex flex-fill flex-column">
                ...
                <div class="mt-2" v-if="symptoms[index] === 'Other'">
                  <Textarea class="form-control h-100px" :value="symptomDesc[index]" label="Description"
                    name="Description" errorMessage="Required" :is-valid="symptomDescValid[index]" :index="index"
                    @handleChange="onSymptomDescChange" :disabled="isDisabled"></Textarea>
                </div>
              </div>
              ...
              <div class="d-flex flex-fill flex-column">
                ...
                <div class="mt-2" v-if="causes[index] === 'Other'">
                  <Textarea :value="causesDesc[index]" label="Description" name="Description" errorMessage="Required"
                    :is-valid="causesDescValid[index]" :index="index" @handleChange="onCauseDescChange"
      ...
      <div class="my-5 w-100 d-flex justify-content-around" v-if="computedDefectStatus">
        <button v-if="!showCancelButtonOnModify" class="btn btn-outline w-100"
          @click.prevent="() => toggleShowDeclineRease(true)"
        ...
        <template v-if="!showCancelButtonOnModify">
          <button class="ms-3 btn btn-warning w-100 modify-button" @click.prevent="handleModifyAndShowCancelButton"
...
const reviseCaption = computed(() => {
  ...
  return `Are you sure want to modify this defect that has already ${status} by the ${role}`
})

const isMonitoring = computed(() => {
  return window.location.href.includes("monitoring-status")
})

const computedDefectStatus = computed(() => {
  if (isMonitoring.value) return false
  let found: Header | undefined = undefined
  if (props.generic) {
    found = formStore.ApprovalData.DefectGenericHeaders.find((x) => {
      return x.defectHeaderId == props.headerId
    })
  } else {
    found = formStore.ApprovalData.DefectHeaders.find((x) => {
      return x.defectHeaderId == props.headerId
    })
  }
  if (found) {
    const isHandledByPlanner = found.plannerStatus == "Acknowledge" || found.plannerStatus == "Decline"
    if (isHandledByPlanner) {
      return false
    }
    const approvalStatus = props.plannerApprove ? found.plannerStatus != 'Acknowledge' : found.status != 'Acknowledge'
    return approvalStatus && found.generalStatus != 'Decline'
  }
  return false
})
...
const submitDecline = async (val) => {
  ...
  if (props.plannerApprove) {
    await formStore.updatePlannerDefectDecline(val, props.generic)
  } else {
    await formStore.updateDefectDecline(val, props.generic)
  }
  loader.close()
...
const handleModifyAndShowCancelButton = (scrollTarget) => {
...
const onAddParts = () => {
  ...
  nextTick(() => {
    handleModifyAndShowCancelButton('partsSection')
  })
  handleScrollToID('partsSection');
}
...
const onAcknowledge = async () => {
  ...
  if (!updateDataOnly.value) {
    if (defectSheetStore.SelectedFitter.name) {
      if (showCancelButtonOnModify.value) {
        const result = await formStore.updateDefectFitter(JSON.stringify(defectSheetStore.SelectedFitter), props.generic, "")
        if (isError.value) {
    ...
    } else if (props.plannerApprove) {
      // jika modify harusnya hit API ini
      ...
      formStore.setCurrentUserGroup("Planner")
      await formStore.updatePlannerDefectAcknowledge({
      ...
      formStore.setCurrentUserGroup("Supervisor")
      await formStore.updateDefectAcknowledge({
        generic: props.generic
      })
    }
  }
  ...
  if (resetDefect.value === true) {
    ...
    formStore.setDefectNewData(!updateDataOnly.value)
  }
...
const onFormOpened = () => {
  ...
  data.value.validatePlannedDuration()
  if (props.plannerApprove) {
    priorityOnPlanner.value = priority.value
  }
}
