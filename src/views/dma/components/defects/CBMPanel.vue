<template>
          ...
          <div class="item px-2 my-2 mx-1">
            <div class="row flex-nowrap px-2 my-2 mx-1 w-100">
              <div
                ...
                {{ displayTaskNoCBM(item.taskNo.replace(';', '')) }}</div>
              <div class="col align-items-center d-flex text-break">
                <span class="text-break defect-identified-task"
                  v-html="displayDescCBM(item.taskDesc, item.taskNo.replace(';', ''))"></span>
              </div>
              <div class="col align-items-center d-flex text-break">
  ...
  <CBMInfo :visibility="formVisibility" :cbm-data="(cbmData as Header)" :cbm-is-confirmed="isCBMConfirmed"
    :view-only="isDisabled" :cbm-data-history="(cbmDataHistory as CBMInfoDetail)" @close-form="onFormClosed"
    @on-approve-cbm="acknowledgeCBM">
  </CBMInfo>
...
import CBMInfo from '@/views/dma/components/defects/CBMInfo.vue'
import {
  useDefectsStore
...
const cbmDataHistory = ref<CBMInfoDetail | undefined>()
...
const showDetail = async (headerId: string) => {
  const header = props.headers.find((h) => {
    return h.id === headerId
  });
  if (header) {
    cbmData.value = header
    cbmDataHistory.value = await store.getDefectsDataCBM(header.workorder || "", header.taskId) as unknown as CBMInfoDetail
  }
  ...
  try {
    ...
    if (header) {
      if (props.plannerApprove) {
        await store.updatePlannerCBMConfirmedStatus(params.headerId, params.reason)
      } else {
        await store.updateCBMConfirmedStatus(params.headerId, params.reason)
      }
      if (params.callback) {
        emits('onRefreshData')
