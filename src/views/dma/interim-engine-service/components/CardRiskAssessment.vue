        ...
        <div class="col-2 col-md-1">
          <div class="row justify-content-center">
            <div class="px-0 col-6">
              <button class="btn btn-sm btn-icon btn-bg-light" :disabled="isButtonDisabled" style="cursor:default">
...
import ImagePreviewPrerisk from '@/components/camera/ImagePreviewPrerisk.vue'
...
watch(fitter, async (newVal) => {
  // reset when fitter change
  callCount.value = 0
  if (formStore.IsFitterLoggedInExist) {
    if (formStore.FitterInfo.employee.id == newVal.id) {
      uploadedImageArr.value = stringToImageInfoConvert(formStore.FitterInfo.riskPhotos).slice(0, 2)
    } else {
...
const uploadedImageArr = ref<ImageInfo[]>([])
...
const handlePostData = async () => {
  ...
  // get time
  const currentTime = await formStore.getServerTimestamp()
  // get emchanic
  const mechanic = eformStore.employee
