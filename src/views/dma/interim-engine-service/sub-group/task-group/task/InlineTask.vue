  ...
  <div v-show="showTask">
    <div class="row w-100" :class="[paddingTopTable, requiredTaskClass]" :style="[generalStyle, taskCustomStyle]">
      <template v-for="item in props.task.items" :key="item.key">
        ...
        <Dropdown v-if="item.itemType === 'dropDown'" :item="item" :task="task" :general-submitted="generalSubmitted" @setLoading="setLoading" />
        <Input v-if="item.itemType === 'input'" :item="item" :task="task" :general-submitted="generalSubmitted" />
        <SmallCamera v-if="item.itemType === 'smallCamera'" :item="item" :task="task" :general-submitted="generalSubmitted" />
