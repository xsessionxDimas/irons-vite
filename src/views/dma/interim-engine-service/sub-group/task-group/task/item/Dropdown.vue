...
import {
  useInterimCracksFormStore
} from '@/store/pinia/dma/interim-engine-service/cracks/useInterimCracksFormStore';
...
const defectPrevStore = useInterimPreviewDefectFormStore();
const store = useInterimEngineStore();
const defectStore = useInterimDefectsFormStore();
const crackStore = useInterimCracksFormStore()
...
const checkSameFitter = computed(() => {
  return checkTaskEditSameFitter(taskPropsItem.value!.updatedBy || "", store.employee)
})
  ...
  if (props.item.categoryItemType == "dropdownTool" || props.item.categoryItemType == "dropdownToolDisc") {
    ...
    if (defectCancel.value || crackCancel.value || naCancel.value) {
      ...
      if (!globalConnectionStore.stateConnectionStatus) {
        ...
        itemRef.value.value = val;
        if (oldValue.value == '3' && props.task?.category === 'NORMAL' && val != '' && !checkSameFitter.value) {
        ...
        } else if (val === '2' && props.task?.category === 'NORMAL') {
        ...
        } else if (props.task!.showParameter == "suspensionCylinder" && val == "Yes") {
          loadingIndicator.value = true
...
const getPrevApprovalData = async (params) => {
  const approvalData = await defectPrevStore.getApprovalDefect(params)
  defectPrevStore.setApprovalData(approvalData || {} as ApprovalData)
}
...
const readOnly = computed(() => {
  ...
  const normalCategory = itemRef.value.value == "3" && taskPropsItem.value?.category == 'NORMAL'

  if (cbmCategory || normalCategory) {
    sameFitter = true
  }

  let defect = false
  if (inputValue.value == '2' || (props.task?.category == 'CRACK' && inputValue.value == '3')) {
    defect = true
  }
  let isSCAdjustmentYes = false
  if (!isUndefined(props.task?.showParameter) && props.task?.showParameter == "suspensionCylinder") {
    if (inputValue.value == 'Yes') {
      isSCAdjustmentYes = true
    }
  }
  return !sameFitter || defect || disabledByOtherItem.value || loadingIndicator.value || isSCAdjustmentYes || isSubmited.value
});
