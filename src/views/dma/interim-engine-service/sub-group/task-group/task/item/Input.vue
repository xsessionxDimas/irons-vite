<template>
  ...
  <!-- <OilNotMapped :show="showOilNotMapped" @close="closeOilNotMappedModal"/> -->
</template>
...
const disabledCondition = computed(() => {
  ...
  return cond || props.task!.isShowAdjustmentRow || dynamicParamRatingUomNotSelected || disabledByOtherItem.value || scHeightInputNDropdownYes
})
...
const readOnly = computed(() => {
  ...
  if (!isUndefined(taskPropsItem.value!.updatedBy!) && !isUndefined(taskPropsItem.value!.updatedBy!.id) && itemRef.value.categoryItemType != "paramRating") {
  ...
  return targetRating || !sameFitter || readOnly || !sameFitterOnItem || !sameFitterOnAverage || isSubmited.value
})
