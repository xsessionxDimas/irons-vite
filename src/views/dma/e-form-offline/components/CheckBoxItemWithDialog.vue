...
import ReasonDialog from '@/views/dma/e-form-offline/components/ReasonDialog.vue';
import { AESTCurrentDateTime } from '@/core/helpers/date-format';
import { Employee } from '@/core/types/entities/dma/e-form/defects/Employee';
import {
...
const onCheckboxChanges = async (reason: { selectedReason: string, desc: string, reset?: boolean}) => {
  ...
  let updateDate = AESTCurrentDateTime()
  if (reason.reset) {
    reasonValue = ""
    checkBoxValue = "false"
    updateDate = ""
  ...
  if (!isOfflineOrSlowInternetConnection()) {
    isError = await generalFormStore.postCheckBeforeTruckItem(updateParam, itemRef.value.key, reason.reset)
  } else {
    await generalFormStore.updateCheckBeforeTruckToLocalDB(updateParam, itemRef.value.key, reasonValue, reason.reset)
    isError = false
  }
  if (!isError) {
