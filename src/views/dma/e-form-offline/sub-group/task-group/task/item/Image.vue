<template>
  <div :class="[widthStyle, potraitWidthStyle, paddingBottomCondition]" class="pt-2 px-2 d-flex d-flex justify-content-center" :style="style" v-loading="loading">
...
const image = ref<string>('/media/icons/bumaau/warning.png')
...
onBeforeMount(async () => {
  try {
    ...
    if (!isOfflineOrSlowInternetConnection()) {
      ...
      const localAsset = await getReferenceDataFromLocalDB(itemRef.value.value) as TaskReference
      ...
      if (getNewImage) {
        const blob = await getImageFromAPI(itemRef.value.value)
        image.value = urlCreator.createObjectURL(blob)
        // dump image
        handleReplaceImageToLocalDB(itemRef.value.value, store.stateGeneralForm.workOrder, blob)
      } else {
        image.value = await getReferenceUrlFromLocalDB(itemRef.value.value)
      }
    } else {
      image.value = await getReferenceUrlFromLocalDB(itemRef.value.value)
    }
