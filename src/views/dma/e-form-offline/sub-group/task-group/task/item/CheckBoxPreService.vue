  ...
  <div :class="[potraitWidthStyle, widthStyle, paddingBottomCondition]" class="pt-2 px-2 d-flex align-items-center d-flex justify-content-center" :style="style">
    <div class="p-2">
      <div class="form-check checkbox-success item-checkbox">
        <input class="form-check-input" type="checkbox" v-model="inputValue" @change="handleReason" :disabled="btnDisabledCondition" >
      </div>
    </div>
  </div>
  <PreServiceReasonDialog :options="store.stateSkipPreServiceReasonOptions" :visibility="viewReasonDialog" @on-save="handleSaveReason" @on-close="handleCloseReason" @on-cancel="handleCancel" />
</template>
...
import PreServiceReasonDialog from "./dialog/PreServiceReasonDialog.vue"
...
const handleSaveReason = (paramReason) => {
  onSave(paramReason)
}
...
const onSave = async (paramReason) => {
  ...
  const taskValue = {
    ...
    propertyParams: [
      {
        propertyName: 'reason',
        propertyValue: paramReason.reason
      },
      {
  ...
  if (!isOfflineOrSlowInternetConnection()) {
    ...
    await store.updateServiceSheetTaskOnLocalDB(payload, props.task!.key, props.item.key, false, props.task!.key)
  }
  store.setSkipPreService(true)
  loading.close()
  paramReason.showToast()
}
...
const btnDisabledCondition = computed(() => {
  ...
  return props.item.value == "true" || store.isAllPreServiceFinished || isSubmited.value || !props.generalSubmitted
})
...
watch(() => {
  ...
  let value = false
  if (newVal == "true") value = true
  store.setSkipPreService(value)
})
