...
<div :class="itemAlignment(column)" class="eform-table-row col px-0" v-for="column in columns" :key="column.key">
  ...
  <Input v-if="column.itemType === 'input'" :item="column" :task="task" :item-loading="itemLoading" :general-submitted="generalSubmitted"/>
  <Dropdown v-if="column.itemType === 'dropDown'" :item="column" :task="task" :item-loading="itemLoading" :general-submitted="generalSubmitted"/>
  <SmallCamera v-if="column.itemType === 'smallCamera'" :item="item" :task="task" :general-submitted="generalSubmitted"/>
  <CheckBox v-if="item.itemType === 'checkbox'" :item="item" :task="task"/>
  <HtmlItem v-if="column.itemType === 'html'" :item="column" :task="task"/>
...
import HtmlItem from "@/components/e-form/HTML.vue";
