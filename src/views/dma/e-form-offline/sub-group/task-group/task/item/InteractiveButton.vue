...
const paleColor = computed(() => {
  let paleColor = ''
  if (props.task!.isShowAdjustmentRow) paleColor = 'opacity: 50%;'
  if (props.task!.isShowReplacementRow) paleColor = 'opacity: 50%;'
...
const buttonDisabled = computed(() => {
  ...
  // kalau nilainya A/B
  let isTaskDefect = props.task!.taskValue == 'A' || props.task!.taskValue == 'B'
  // untuk kasus cbm manual cleaned, yang di disabled jika input === A
  if (props.task!.rating == TaskRatingEnum.MANUAL_CLEANED) {
    isTaskDefect = props.task!.taskValue == 'A'
  }
  ...
  return istaskNotFilled || isTaskDefect || props.task!.isShowAdjustmentRow || !sameFitter || props.isShowAdjustment || isSubmited.value || props.task!.isShowReplacementRow || props.task!.isShowCleanedRow
})
...
const checkReplacementValue = async () => {
  if (!taskRef.value?.replacement.measurement) {
    const payload = {
      modelId: store.stateGeneralForm.modelId,
      psTypeId: store.stateGeneralForm.psTypeId,
      taskId: taskRef.value?.key
    }
    const replacement = taskRef.value?.replacement
    if (replacement) {
      replacement.measurement = await store.getReplacementDefault(payload)
    }
  }
}
...
const handleClick = () => {
  if (itemRef.value.value == 'Adjustment') {
    ...
    emits("showTaskAdjustment", true)
  } else if (itemRef.value.value === 'Replaced') {
    checkReplacementValue().then(() => {
      if (taskRef.value) {
        taskRef.value.isShowReplacementRow = true
        emits("showTaskAdjustment", true)
      }
    })
  } else if (itemRef.value.value == 'Cleaned') {
    if (taskRef.value) {
      taskRef.value.isShowCleanedRow = true;
    }
    emits("showTaskAdjustment", true)
  }
}
