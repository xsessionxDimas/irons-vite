...
const targetValue = computed(() => {
  ...
  if (naStore.stateSuccessUpdateItemKey == "") {
    value = ""
  }
  return value
})
  ...
  if (!isUndefined(props.task.disabledByItemKey)) {
    ...
    if (isUndefined(disabledItem)) {
      ...
      if (!isOfflineOrSlowInternetConnection()) {
        value = await store.getValueFromItemKey(props.task.disabledByItemKey)
      } else {
        value = await store.getFieldValueByKey(props.task.disabledByItemKey, "value")
      }
    ...
    if (disabledItem?.value == '3') {
        ...
        for (var prop in itemProp) {
          ...
          if (!isUndefined(item.disabledByItemKey)) {
            if (item.value) {
              ...
              deleteByWOItemKey(store.stateGeneralForm.workOrder, item.key)
            }
          }
        }
      })
      ...
      if (updateParams.length > 0) {
        ...
        await store.updateItemFromTask(updateParams, props.task.key)
        if (isOfflineOrSlowInternetConnection()) {
          store.updateSelectedSubGroupFromServiceSheets()
        }
