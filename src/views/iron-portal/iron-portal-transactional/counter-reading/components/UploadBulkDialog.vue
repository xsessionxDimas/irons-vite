<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataCounterReading.xlsx" target="_blank" id="link-download">here</a>
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      smu: val.smu,
      counterReading: val.counterReading,
      remark: val.validationReason,
      status: val.isValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("CounterReading", columns, data);
}
