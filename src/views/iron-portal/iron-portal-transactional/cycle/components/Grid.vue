    ...
    >
      ...
      <el-table-column prop="site" label="Site" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="{row}">
          <AutoComplete
            v-if="row.cycleId === editedItem.cycleId"
      ...
      <el-table-column prop="equipmentModel" label="Model" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="{row}">
          <AutoComplete
            v-if="row.cycleId === editedItem.cycleId"
      ...
      <el-table-column prop="equipmentNumber" label="Equipment" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="{row}">
          <AutoComplete
            v-if="row.cycleId === editedItem.cycleId"
            ...
            @handleChange="(val) => editedItem.equipmentNumber = val"
      ...
      <el-table-column prop="hoCycleTarget" label="Ho Cycle Target" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="{row}">
          <DecimalInput
            v-if="row.cycleId === editedItem.cycleId"
            :value="editedItem.hoCycleTarget"
            placeholder="Add Ho Cycle Target"
            name="hoCycleTarget"
            @handleChange="(val) => editedItem.hoCycleTarget = val" />
          <span v-else>{{ row.hoCycleTarget }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="siteCycleTarget" label="Site Cycle Target" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="{row}">
          <DecimalInput
            v-if="row.cycleId === editedItem.cycleId"
            :value="editedItem.siteCycleTarget"
            placeholder="Add Site Cycle Target"
            name="siteCycleTarget"
            @handleChange="(val) => editedItem.siteCycleTarget = val" />
          <span v-else>{{ row.siteCycleTarget }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="startDate" label="Start Date" align="center" width="170" sortable>
        <template #default="scope">
          <DatePickerInput
          v-if="scope.row.cycleId === editedItem.cycleId"
      ...
      <el-table-column label="" width="80">
        <template #default="scope">
            <div v-if="scope.row.cycleId !== editedItem.cycleId">
              <el-tooltip
              class="box-item"
              effect="dark"
...
const inputValidation = Yup.object().shape({
  ...
  hoCycleTarget: Yup.string().required("Ho Cycle Target is mandatory"),
  siteCycleTarget: Yup.string().required("Site Cycle Target is mandatory"),
