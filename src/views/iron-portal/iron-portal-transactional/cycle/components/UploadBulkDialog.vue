<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataCycle.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-transactional/cycle/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      hoCycleTarget: item.hoCycleTarget,
      siteCycleTarget: item.siteCycleTarget,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      hoCycleTarget: val.hoCycleTarget,
      siteCycleTarget: val.siteCycleTarget,
  ...
  handleDataToExcel("Cycle", columns, data);
}
