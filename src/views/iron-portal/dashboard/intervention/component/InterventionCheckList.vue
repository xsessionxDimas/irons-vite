<template>
  ...
  <el-table
    ...
    >
    <el-table-column
      reserve-selection class="table-selection"
      class-name="table-selection"
      type="selection"
      :width="52"
      align="right"
      header-align="right"
      :selectable="() => { return !props.disableSelection}"
    />
    <el-table-column class-name="table-number" label="No" width="50" align="center">
...
const imageValues = ref<ImageInfo[]>([]);
...
const handleSelectionChange = async (rows) => {
  console.log("checks row", rows)
  listStore.updateInterventionCheckListMandatory(rows)
}
...
const setSelection = () => {
  ...
  if (props.listData.length > 0) {
    const itemsMandatory = props.listData.filter((item) => {
      return item.isMandatory == true
    })
    itemsMandatory.forEach((row) => {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      tableRecActionRef.value!.toggleRowSelection(row, true)
    })
  } else {
    console.log("ELSE CLEAR SELECTION")
    tableRecActionRef.value!.clearSelection()
  }
  console.log("**************************************************")
}
...
.table-selection.el-table-column--selection{
