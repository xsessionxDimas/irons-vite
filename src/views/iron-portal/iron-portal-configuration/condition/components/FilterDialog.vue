<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.condition"
        :value-to="filterData.conditionTo"
        label="Condition"
        name="Condition"
        :options="listStore.conditionOption"
        @handle-change-from="onConditionSelected"
        @handle-change-to="onConditionToSelected"
     />
      <DatePickerInput
...
const onConditionSelected = (value: string) => {
  listStore.setCondition(value);
};
const onConditionToSelected = (value: string) => {
  listStore.setConditionTo(value);
};
...
const handleFilterClick = () => {
  const checkCondition = listStore.lastUsedFilterData.condition !== listStore.filterData.condition
  const checkConditionTo = listStore.lastUsedFilterData.conditionTo !== listStore.filterData.conditionTo
  ...
  const status = (checkCondition || checkConditionTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
