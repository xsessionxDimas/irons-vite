    ...
    >
      ...
      <el-table-column prop="site" label="Site" min-width="170" sortable :sort-method="() => defaultSorting(columnOrder)">
        <template #default="{row}">
          <AutoComplete
            v-if="row.mappingSosId === editedItem.mappingSosId"
      ...
      <el-table-column prop="compartId" label="Compart" min-width="170" sortable :sort-method="() => defaultSorting(columnOrder)">
        <template #default="{row}">
          <TextInput
            v-if="row.mappingSosId === editedItem.mappingSosId"
            :value="editedItem.compartId"
            placeholder="Add Compart"
            name="CompartId"
            @handleChange="(val) => editedItem.compartId = val" />
          <span v-else>{{ row.compartId }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="component" label="Component" min-width="190" sortable :sort-method="() => defaultSorting(columnOrder)">
        <template #default="{row}">
          <TextInput
            v-if="row.mappingSosId === editedItem.mappingSosId"
      ...
      <!-- <el-table-column prop="startDate" label="Start Date" align="center" width="170" sortable>
        <template #default="scope">
          <DatePickerInput
          v-if="scope.row.mappingSosId === editedItem.mappingSosId"
      ...
      <el-table-column prop="isActive" label="Is Active" :width="editedItem.mappingSosId != 0 ? 350 : 100" sortable align="center">
        <template #default="scope">
          <div
            v-if="scope.row.mappingSosId === editedItem.mappingSosId"
      ...
      <el-table-column label="" width="100">
        <template #default="scope">
          <div v-if="scope.row.mappingSosId !== editedItem.mappingSosId" class="d-flex justify-content-end">
