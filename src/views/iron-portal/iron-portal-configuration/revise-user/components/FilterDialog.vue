<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.reviseUser"
        :value-to="filterData.reviseUserTo"
        label="Revise User"
        name="ReviseUser"
        :options="listStore.reviseUserOption"
        @handle-change-from="onReviseUserSelected"
        @handle-change-to="onReviseUserToSelected"
     />
      <DatePickerInput
...
import {
  useReviseUserListStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/revise-user/useReviseUserListStore";
import {
  FilterData
...
/* stores */
const listStore = useReviseUserListStore();
...
const onReviseUserSelected = (value: string) => {
  listStore.setReviseUser(value);
};
const onReviseUserToSelected = (value: string) => {
  listStore.setReviseUserTo(value);
};
...
const handleFilterClick = () => {
  const checkReviseUser = listStore.lastUsedFilterData.reviseUser !== listStore.filterData.reviseUser
  const checkReviseUserTo = listStore.lastUsedFilterData.reviseUserTo !== listStore.filterData.reviseUserTo
  ...
  const status = (checkReviseUser || checkReviseUserTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
