...
import {
  useReviseUserBulkStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/revise-user/useReviseUserBulkStore";
import {
  useReviseUserFormStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/revise-user/useReviseUserFormStore";
import { saveAs } from "file-saver";

const store = useStore();
const listStore = useReviseUserListStore();
const formStore = useReviseUserFormStore();
const bulkStore = useReviseUserBulkStore();
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Revise User", [
    {
    ...
    },
    {
      ...
      pageRoute: "ironlake-config-reviseuser",
    },
    {
      pageName: "Revise User",
      pageRoute: "ironlake-config-reviseuser",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getLookupEmployee();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `ReviseUser.xlsx`);
}
