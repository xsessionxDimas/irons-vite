<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataDamageCategory.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useDamageCategoryBulkStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/damage-category/useDamageCategoryBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/damage-category/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/damage-category/BulkItem";
...
const bulkStore = useDamageCategoryBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      damageCategoryDescription: item.DamageCategoryDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      damageCategory: val.DamageCategory,
      damageCategoryDescription: val.DamageCategoryDescription,
  ...
  handleDataToExcel("DamageCategory", columns, data);
}
