<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.week"
        :value-to="filterData.weekTo"
        label="Week"
        name="Week"
        :options="listStore.weekOption"
        @handle-change-from="onWeekSelected"
        @handle-change-to="onWeekToSelected"
     />
      <DatePickerInput
...
const onWeekSelected = (value: string) => {
  listStore.setWeek(value);
};
const onWeekToSelected = (value: string) => {
  listStore.setWeekTo(value);
};
...
const handleFilterClick = () => {
  const checkWeek = listStore.lastUsedFilterData.week !== listStore.filterData.week
  const checkWeekTo = listStore.lastUsedFilterData.weekTo !== listStore.filterData.weekTo
  ...
  const status = (checkWeek || checkWeekTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
