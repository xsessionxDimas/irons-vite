<template>
  ...
  <template v-else>
    ...
    >
      ...
      <el-table-column prop="Action" label="Action" min-width="170" sortable :sort-method="(a, b) => customSortNumber(a, b, 'Action')" />
      <el-table-column prop="ActionDescription" label="Action Description" min-width="170" sortable :sort-method="(a, b) => customSort(a, b, 'ActionDescription')" />
...
import {
  useActionListStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/action/useActionListStore";
import {
  useActionFormStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/action/useActionFormStore";
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/action/ListItem";
import {
  PropType,
  defineProps,
  defineEmits,
...
const listStore = useActionListStore();
...
const customSortNumber = (objA, objB, field) => {
  return dynamicSortNumber(objA[field], objB[field], columnOrder.value);
}
