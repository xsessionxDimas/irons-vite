<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.action"
        :value-to="filterData.actionTo"
        label="Action"
        name="Action"
        :options="listStore.actionOption"
        @handle-change-from="onActionSelected"
        @handle-change-to="onActionToSelected"
     />
      <DatePickerInput
...
const onActionSelected = (value: string) => {
  listStore.setAction(value);
};
const onActionToSelected = (value: string) => {
  listStore.setActionTo(value);
};
...
const handleFilterClick = () => {
  const checkAction = listStore.lastUsedFilterData.action !== listStore.filterData.action
  const checkActionTo = listStore.lastUsedFilterData.actionTo !== listStore.filterData.actionTo
  ...
  const status = (checkAction || checkActionTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
