<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataElement.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useElementBulkStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/element/useElementBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/element/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/element/BulkItem";
...
const bulkStore = useElementBulkStore();
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      element: val.Element,
      elementDescription: val.ElementDescription,
  ...
  handleDataToExcel("Element", columns, data);
}
