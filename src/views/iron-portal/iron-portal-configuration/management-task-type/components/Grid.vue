<template>
  ...
  <template v-else>
    ...
    >
      ...
      <el-table-column prop="taskTypeCode" label="Task Type Code" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="scope">
          <ElementTextInput
            v-if="scope.row.riskManagementTaskTypeId === editedItem.riskManagementTaskTypeId"
            :value="editedItem.taskTypeCode"
            placeholder="Add Task Type Code"
            name="TaskTypeCode"
            :disabled="true"
          />
          <span v-else>{{ scope.row.taskTypeCode }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taskTypeDescription" label="Task Type Description" min-width="170" sortable :sort-method="defaultSorting(columnOrder)">
        <template #default="scope">
          <ElementTextInput
            v-if="scope.row.riskManagementTaskTypeId === editedItem.riskManagementTaskTypeId"
            :value="editedItem.taskTypeDescription"
            placeholder="Add Task Type Description"
            name="taskTypeDescription"
            :max="80"
            @handleChange="(val) => editedItem.taskTypeDescription = val"
          />
          <span v-else>{{ scope.row.taskTypeDescription }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="" width="100">
        <template #default="scope">
          <div v-if="scope.row.riskManagementTaskTypeId !== editedItem.riskManagementTaskTypeId" class="d-flex justify-content-end">
            <el-tooltip
              class="box-item"
              effect="dark"
...
const inputValidation = Yup.object().shape({
  taskTypeCode: Yup.string().required("Task Type Code is mandatory"),
  taskTypeDescription: Yup.string().required("Task Type Description is mandatory"),
