<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataParameterObject.xlsx" target="_blank" id="link-download">here</a>
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      parameterObjectId: 0,
      parameterObject: item.ParameterObject,
      parameterObjectDescription: item.ParameterObjectDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      parameterObject: val.ParameterObject,
      parameterObjectDescription: val.ParameterObjectDescription,
  ...
  handleDataToExcel("ParameterObject", columns, data);
}
