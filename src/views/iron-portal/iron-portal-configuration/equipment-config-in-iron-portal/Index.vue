...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Equipment Config In IronPortal", [
    {
    ...
    },
    {
      ...
      pageRoute: "ironlake-config-equipmentconfiginIronportal",
    },
    {
      pageName: "Equipment Config In IronPortal",
      pageRoute: "ironlake-config-equipmentconfiginironportal",
    },
  ]);
  listStore.setPage(1);
  ...
  await formStore.getLookupCodeGroup();
  await formStore.getLookupSite();
  await formStore.getLookupDamage();
  await formStore.getLookupDamageCategory();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `EquipmentConfigInIronPortal.xlsx`);
}
