<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataRiskRankMaterial.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/risk-rank-material/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      riskRankIntervalId: 0,
      maxRiskRank: item.MaxRiskRank,
      revisedRisk: item.RevisedRisk,
...
const handleDownload = () => {
  ...
  handleDataToExcel("Rating", columns, data);
}
