...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("Component Life Threshold", [
    {
    ...
    },
    {
      ...
      pageRoute: "ironlake-config-componentcategory",
    },
    {
      pageName: "Component Life Threshold",
      pageRoute: "ironlake-config-componentcategory",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getLookupType();
  await formStore.getLookupUom();
  await formStore.getLookupOperator();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `ComponentLifeThreshold.xlsx`);
}
