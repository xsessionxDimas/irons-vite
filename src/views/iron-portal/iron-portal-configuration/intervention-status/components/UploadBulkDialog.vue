<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataInterventionStatusandFollowUpPriority.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/intervention-status/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/intervention-status/BulkItem";
...
const handleDownload = () => {
  ...
  const columns = [
    "INTERVENTION STATUS",
    "INTERVENTION STATUS DESC",
    "FOLLOW UP PRIORITY",
    "FOLLOW UP PRIORITY UOM",
    "START DATE",
    "END DATE",
    "REMARK",
    "STATUS",
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      interventionStatusDesc: val.interventionStatusDesc,
      followUpPriority: val.followUpPriority,
      followUpPriorityUom: val.followUpPriorityUom,
  ...
  handleDataToExcel("InterventionStatus", columns, data);
}
