<template>
  ...
  <template v-else>
    ...
    >
      ...
      <el-table-column prop="Damage" label="Damage" min-width="170" sortable :sort-method="(a, b) => customSort(a, b, 'Damage')" />
      <el-table-column prop="DamageDescription" label="Damage Description" min-width="170" sortable :sort-method="(a, b) => customSort(a, b, 'DamageDescription')" />
      ...
      <el-table-column label="" width="80">
        <template #default="scope">
          <el-tooltip class="box-item" effect="dark" :content="scope.row.IsActive ? 'Edit' : 'Inactive data cannot be edited'" placement="top">
            <div class="row justify-content-center">
              <button :disabled="!scope.row.IsActive" class="btn btn-sm btn-icon btn-bg-light" @click="handleEditRow(scope.row)">
...
import {
  useDamageListStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/damage/useDamageListStore";
import {
  useDamageFormStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/damage/useDamageFormStore";
import {
  ListItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/damage/ListItem";
import {
  PropType,
  defineProps,
  defineEmits,
...
const listStore = useDamageListStore();
const formStore = useDamageFormStore();
