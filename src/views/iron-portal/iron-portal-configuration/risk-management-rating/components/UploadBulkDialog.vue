<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
        Download template
        <a href="documents/MasterDataComponentRiskRatingDescription.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/risk-management-rating/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ratingParameter: item.ratingParameter,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ratingParameter: val.ratingParameter,
      ratingCode: val.ratingCode,
  ...
  handleDataToExcel("ComponentRiskRatingDescription", columns, data);
}
