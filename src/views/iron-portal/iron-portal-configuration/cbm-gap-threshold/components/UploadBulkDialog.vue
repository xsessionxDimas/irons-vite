<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataCbmComplianceGapThreshold.xlsx" target="_blank" id="link-download">here</a>
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/cbm-gap-threshold/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/cbm-gap-threshold/BulkItem";
...
const bulkStore = useCbmGapThresholdBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      valueThreshold: item.valueThreshold,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      valueThreshold: val.valueThreshold,
  ...
  handleDataToExcel("CbmGapThreshold", columns, data);
}
