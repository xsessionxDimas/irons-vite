<template>
  ...
  <div class="card">
    <div class="card-body grid-padding">
      <Grid :list-data="listData" :page="pagination.currentPage" @show-dialog="handleShowFormEditDialog" @fetch-data="reload"/>
    </div>
  </div>
  <div class="my-5">
...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("CBM Compliance Gap Threshold", [
    ...
    },
    {
      ...
      pageRoute: "ironlake-config-cbmgapthreshold",
    },
    {
      pageName: "CBM Compliance Gap Threshold",
      pageRoute: "ironlake-config-cbmgapthreshold",
    },
  ]);
  listStore.setPage(1);
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `CBMComplianceGapThreshold.xlsx`);
}
