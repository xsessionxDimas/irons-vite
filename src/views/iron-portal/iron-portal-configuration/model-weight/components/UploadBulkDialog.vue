<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataModelPriceWeight.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/model-weight/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      weight: item.weight,
      whPerday: item.whPerday,
      fuelBurn: item.fuelBurn,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      weight: val.weight,
      whPerday: val.whPerday,
      fuelBurn: val.fuelBurn,
  ...
  handleDataToExcel("ModelPriceWeight", columns, data);
}
