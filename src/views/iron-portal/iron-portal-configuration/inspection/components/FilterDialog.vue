<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.inspection"
        :value-to="filterData.inspectionTo"
        label="Inspection"
        name="Inspection"
        :options="listStore.inspectionOption"
        @handle-change-from="onInspectionSelected"
        @handle-change-to="onInspectionToSelected"
     />
      <DatePickerInput
...
const onInspectionSelected = (value: string) => {
  listStore.setInspection(value);
};
const onInspectionToSelected = (value: string) => {
  listStore.setInspectionTo(value);
};
...
const handleFilterClick = () => {
  const checkInspection = listStore.lastUsedFilterData.inspection !== listStore.filterData.inspection
  const checkInspectionTo = listStore.lastUsedFilterData.inspectionTo !== listStore.filterData.inspectionTo
  ...
  const status = (checkInspection || checkInspectionTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
