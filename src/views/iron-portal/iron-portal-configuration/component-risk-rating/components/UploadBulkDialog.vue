<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataComponentRiskRating.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/component-risk-rating/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      oemInterval: item.oemInterval,
      bumaAuTarget: item.bumaAuTarget,
      maxRiskRank: item.maxRiskRank,
      revisedRank: item.revisedRank,
      overallRisk: item.overallRisk,
      failureTiming: item.failureTiming,
      systemImpact: item.systemImpact,
      opsImpact: item.opsImpact,
      supplyRisk: item.supplyRisk,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      componentWeight: val.componentWeight,
      oemInterval: val.oemInterval,
      bumaAuTarget: val.bumaAuTarget,
      maxRiskRank: val.maxRiskRank,
      revisedRank: val.revisedRank,
      overallRisk: val.overallRisk,
      failureTiming: val.failureTiming,
      systemImpact: val.systemImpact,
      opsImpact: val.opsImpact,
      supplyRisk: val.supplyRisk,
      failureCost: val.failureCost,
  ...
  handleDataToExcel("Component Risk Rating", columns, data);
}
