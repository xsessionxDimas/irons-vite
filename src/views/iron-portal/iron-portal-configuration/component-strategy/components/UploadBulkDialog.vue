<template>
  ...
  >
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataComponentStrategy.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/component-strategy/BulkItem";
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      strategyTaskDesc: item.strategyTaskDesc,
      oemInterval: item.oemInterval,
      percentageOfOem: item.percentageOfOem,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      strategyTaskDesc: val.strategyTaskDesc,
      oemInterval: val.oemInterval,
      percentageOfOem: val.percentageOfOem,
  ...
  handleDataToExcel("ComponentStrategy", columns, data);
}
