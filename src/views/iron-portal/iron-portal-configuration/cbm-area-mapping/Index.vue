...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("CBM Area Mapping", [
    {
    ...
    },
    {
      ...
      pageRoute: "ironlake-config-cbmareamapping",
    },
    {
      pageName: "CBM Area Mapping",
      pageRoute: "ironlake-config-cbmareamapping",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getLookupCbmMapping();
  await formStore.getLookupArea();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `CbmAreaMapping.xlsx`);
}
