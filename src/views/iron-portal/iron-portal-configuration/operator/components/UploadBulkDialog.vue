<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataOperator.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useOperatorBulkStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/operator/useOperatorBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/operator/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/iron-portal/iron-portal-configuration/operator/BulkItem";
...
const bulkStore = useOperatorBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      operatorDescription: item.OperatorDescription,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      operatorDescription: val.OperatorDescription,
  ...
  handleDataToExcel("Operator", columns, data);
}
