<template>
  ...
  >
    <div class="row g-9 my-4">
      <AutoComplete
        :value-from="filterData.item"
        :value-to="filterData.itemTo"
        label="Item"
        name="Item"
        :options="listStore.itemOption"
        @handle-change-from="onItemSelected"
        @handle-change-to="onItemToSelected"
     />
      <DatePickerInput
...
import {
  useItemListStore
} from "@/store/pinia/iron-portal/iron-portal-configuration/item/useItemListStore";
import {
  FilterData
...
/* stores */
const listStore = useItemListStore();
...
const onItemSelected = (value: string) => {
  listStore.setItem(value);
};
const onItemToSelected = (value: string) => {
  listStore.setItemTo(value);
};
...
const handleFilterClick = () => {
  const checkItem = listStore.lastUsedFilterData.item !== listStore.filterData.item
  const checkItemTo = listStore.lastUsedFilterData.itemTo !== listStore.filterData.itemTo
  ...
  const status = (checkItem || checkItemTo || checkStartDate || checkStartDateTo || checkEndDate || checkEndDateTo)
