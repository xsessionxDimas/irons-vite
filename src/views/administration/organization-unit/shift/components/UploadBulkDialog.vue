<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataShift.xlsx" target="_blank" id="link-download">here</a>
...
import {
  useShiftBulkStore
} from "@/store/pinia/administration/organization-unit/shift/useShiftBulkStore";
import {
...
import {
  ValidatedItem
} from "@/core/types/entities/administration/organization-unit/shift/ValidatedItem";
import {
  BulkItem
} from "@/core/types/entities/administration/organization-unit/shift/BulkItem";
...
const bulkStore = useShiftBulkStore();
...
const mapToBulkItem = () => {
  const data = mapObject(bulkStore.validatedData, (data) => {
    ...
    return {
      ...
      startHourType: item.startHourType,
      endHour: item.endHour,
      endHourType: item.endHourType,
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      startHourType: val.startHourType,
      endHour: val.endHour,
      endHourType: val.endHourType,
  ...
  handleDataToExcel("Shift", columns, data);
}
