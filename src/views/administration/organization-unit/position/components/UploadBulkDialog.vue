<template>
  <el-dialog
    ...
    <div class="mb-3">
      <span>
         Download template
        <a href="documents/MasterDataPosition.xlsx" target="_blank" id="link-download">here</a>
...
import {
  BulkItem
} from "@/core/types/entities/administration/organization-unit/position/BulkItem";
...
const handleDownload = () => {
  ...
  const data = bulkStore.validatedData.map((val) => {
    return {
      ...
      positionDescription: val.PositionDescription,
      startDate: val.StartDate,
      endDate: val.EndDate,
      IsDma: String(val.IsDma),
      remark: val.ValidationReason,
      status: val.IsValid ? "OK" : "Error",
    };
  });
  handleDataToExcel("Position", columns, data);
}
