...
onBeforeMount(async () => {
  ...
  setCurrentPageBreadcrumbs("User Group Menu", [
    {
      pageName: "Ironlake",
      pageRoute: "ironlake",
    },
    {
      ...
      pageRoute: "ironlake-rolemanagement-usergroupmenu",
    },
    {
      pageName: "User Group Menu",
      pageRoute: "ironlake-rolemanagement-usergroupmenu",
    },
  ]);
  listStore.setPage(1);
  await listStore.getLookup();
  await formStore.getMenu();
});
...
const handleDownload = async () => {
  ...
  saveAs(new Blob([blob],
    { type: "application/octet-stream" }), `UserGroupMenu.xlsx`);
};
</script>
